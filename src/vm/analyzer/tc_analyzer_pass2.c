 ...
 */
static int tc_pass2_resolve_target_type(TcInitHistory *hist, const TcType *owned,
...
static int tc_precheck_rhs_names(TcRhs *rhs, const TcSymbolTable *visible,
    ...
    case TC_RHS_SELF_MEMBER:
        /* 0.0.39 Phase 2：可解析；深度类型/执行在后续阶段 */
        return 0;
    }
    return 0;
}
...
/* tc_check_io_format 实现已迁移至 tc_analyze_6e.c（6e 子阶段） */
/* 声明保留于 tc_analyzer_internal.h */

/*
 * @brief 对 RHS 进行类型检查
 * @param expected  赋值目标的期望类型（非 NULL）
