...
int tc_aot_emit_statement_impl(FILE *out, const TcStatement *stmt, TcAotEmitCtx *ctx,
    ...
    if (stmt->kind == TC_STMT_PTR_STORE) {
        ...
        fprintf(out,
                "%s    if (tc_aot_ptr_store(%s, _ptr, _val, %s, "
                "tc_aot_cur_diag, %d) != 0)\n",
                abort_indent, tc_aot_slots_arg(ctx), tc_aot_type_enum(store->pointee_type.tag),
                store->line);
    ...
    if (stmt->kind == TC_STMT_MEMCOPY_UNSAFE) {
        ...
        fprintf(out,
                "%s    if (tc_aot_memcopy_unsafe(%s, _dptr, _d_idx, %s, "
                "_sptr, _s_idx, %s, _len, %zu, %s, tc_aot_cur_diag, %d) != 0)\n",
                abort_indent, tc_aot_slots_arg(ctx),
                tc_aot_type_enum(tc_aot_memcopy_index_type(&mc->dst_index, ctx, stmt_index)),
