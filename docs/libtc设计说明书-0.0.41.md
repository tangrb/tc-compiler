...
0.0.41 编译器标准 §11.4 是 `TcErrorKind` 的权威清单。错误码按阶段分为：
...
> 本节为调用者速查，原独立文件 `libtc-api-0.0.41.md` 已并入本文以消除 API 双源漂移（见《语言标准符合性检查分析报告》M-15）。函数签名以本节为准；内部架构见 §1–§14。错误码分类计数待修复计划 P5 对齐（M-17）。
