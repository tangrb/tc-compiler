...
> **当前版本**：TC-VM v0.0.43（`TC_VM_VERSION` ⇐ `TC_VERSION_CORE`）
>
> **目标语言规范（唯一权威）**：[TC 0.0.44](./TC语言标准设计说明书-0.0.44.md)
>
> **文档职责**：本页准确描述 0.0.44 可执行文件行为（实现版本 v0.0.43）与兼容边界。
>
> **内部架构**：[TC-VM 详细设计说明书](./TC-VM详细设计说明书-0.0.44.md)
...
7. [性能计时](#7-性能计时)
8. [当前示例](#8-当前示例)
9. [0.0.44 迁移与兼容边界](#9-0044-迁移与兼容边界)
10. [测试脚本约定](#10-测试脚本约定)
...
`tc-vm` 是 TC 源文件的直接执行引擎。0.0.44 提供：
...
0.0.44 没有活跃 `TcWarningKind`。`TcWarningList` 仍在内部/API 结构中，但正常编译不产生 warning 行。
...
以下示例适用于 v0.0.43。每个源文件须以 `#program` 或 `#lib` 开头（[语言标准 §4](./TC语言标准设计说明书-0.0.44.md)）；未写模式指令为语法错误。
...
### 9.2 0.0.44 同步状态（实现侧标注）

本页按 0.0.44 规范口径书写（**文档先行**）。命令行行为本身不因 0.0.44 版本升级而改变（规范为**口径收敛版**，不新增语言能力、不改变书写方式）；与 `tc-vm` 可观察行为相关的落地差异如下（同步项登记于《TC 0.0.42 语言标准一致性勘误清单》§9）：

| 条目 | 影响 | 状态 |
| ---- | ---- | ---- |
| 首个规范诊断的**四步选取顺序**（LT → SYN → SEM → CT；同阶段按源序位置；同位置专用码优先；条款内既有顺序优先） | `-e/--print-error-code` 打印的首行码名 | **待同步（W-11／W-27）**：CT 类诊断须挂起至全部 SEM 类诊断无触发后再报告 |
| 指针 `cast` **不附加等宽条件**（D14） | 合法程序集合（影响 `tc-vm` 是否接受该文件） | **待同步（W-6）**：现仍报 `pointer cast requires equal-width pointee types` |
| `read` 标准输入读取失败 → `TC_RE_IO`（D31） | 退出码与错误行 | **已同步**（共享 `tc_io`） |
...
*当前命令行为以 v0.0.43 源码与本页为准，语言规则以 [TC 0.0.44 标准](./TC语言标准设计说明书-0.0.44.md) 为准。*
