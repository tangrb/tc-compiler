...
> **目标版本**：v0.0.26  
> **依据**：`docs/TC语言标准设计说明书.md`（v0.0.25 已实现）→ `docs/TC语言标准设计说明书_0.0.26.md`（目标标准）  
> **范围**：libtc（编译 API）+ TC-VM（解释器）+ TC-AOT（静态编译）

---

## 当前状态（v0.0.25）vs 目标（v0.0.26）
...
| M | 内容 | 前置 | 工期 |
|---|---|---|---|
| M1 | **基础设施**：TcStmtKind / TcErrorKind 枚举、TcWarningKind 移除、版本号 | — | 2–3 天 |
| M2 | **词法/语法**：goto/label 关键字 Token、AST 构建、缩进规则 | M1 | 3–5 天 |
| M3 | **符号表**：标签表管理（TcSymbolTable 扩展）、块级作用域 push/pop | M1 | 3–5 天 |
| M4 | **标签验证**：标签解析 + 4 种跳转合法性判定（块路径栈） | M2, M3 | 3–5 天 |
| M5 | **数据流分析**：未初始化变量 → 静态错误（控制流合并、goto CFG） | M3 | 5–7 天 |
| M6 | **VM 执行**：goto IP 跳转、标签零成本执行 | M4 | 2–3 天 |
| M7 | **AOT 代码生成**：标签/跳转指令 | M4 | 2–3 天 |
| M8 | **测试**：正/反向 goto 用例、未初始化迁移、全量通过 | M5, M6, M7 | 3–5 天 |
| M9 | **文档同步**：标准说明书、VM 详设、知识图谱 | M8 | 2–3 天 |
