...
| 语义归语言标准 | 算术、cast、溢出等行为以《TC 语言标准设计说明书》为准；本文档只规定**实现架构** |
| 单作用域 | v1.0 假定全局单作用域，与 v0.7 语言范围一致 |
...
- 不定义、不生成第二套字节码指令集
- 不做 JIT / LLVM 后端
- 不实现 §12 后续扩展表中预留的控制流、函数、数组等扩展（仅预留接口）
...
| 组件 | 输入 | 输出 | 职责 |
|------|------|------|------|
| **Lexer** | 源文本 | `Token` 流 | 按 §2.2 词法规则切分；记录行号、列号 |
| **Parser** | `Token` 流 | `StatementRecord`（未完全类型化） | 按 §2.3 语法解析单条语句 |
| **Analyzer** | `StatementRecord` 列表 | 已类型化程序 + 符号表 | 符号收集、类型检查、字面量范围、overflow 合法性 |
...
TC v0.7 中，每条合法语句对应一条「高级指令」：

| 源形式 | 指令含义 |
|--------|----------|
| `var x: T = <rhs>` | **DEF** — 定义槽位 `x` 为类型 `T` 并初始化 |
| `x = <rhs>` | **MOV** — 将 `<rhs>` 结果写入已定义槽位 `x` |
| `<rhs> = integer_literal` | 加载常量 |
| `<rhs> = add(T, …)` | **ADD** — 类型 `T`，模式 strict/overflow |
| `<rhs> = sub/mul/div/mod(T, …)` | 对应算术指令 |
| `<rhs> = cast(T, …)` | **CAST** — 目标类型 `T`，模式 strict/overflow |
...
语言标准 EBNF 对空行、文件尾换行未完全规定；**TC-VM v1.0 实现约定**如下（不影响 TC 语义，仅规范实现）：
...
OverflowMode ::= Strict | Overflow   // 关键字 overflow 映射为 Overflow
...
Operand ::= Variable(name: string)
          | Literal(value: uint64)    // 非负整数；类型由上下文 IntType 约束
```
...
Rhs ::= LitRhs { value: uint64 }
      | ArithRhs { op: ArithOp, type: IntType, mode: OverflowMode, lhs: Operand, rhs: Operand }
      | CastRhs { target: IntType, mode: OverflowMode, source: string /* 变量名 */ }
```
...
```text
StatementRecord ::=
    | VarDef { line, name, type: IntType, rhs: Rhs }
...
- 仅在 `VarDef` 时插入；重复 `var` 同名 → **重复定义错误**
- `Assign` / 操作数 / cast 源 引用的 name 必须存在 → 否则 **未定义变量错误**
...
按 `VarDef` 在程序中出现的顺序分配 `slot`：0, 1, 2, …  
Analyzer 第一遍扫描所有 `VarDef` 建立符号表；第二遍做类型检查。
...
```text
analyze(program):
    pass1_collect_symbols(program)   // 仅 VarDef
    pass2_type_check(program)          // 全语句
    return TypedProgram
```
...
|--------|------|
| Assign 目标 | 必须已定义 |
| 左值类型 vs rhs | `VarDef` / `Assign` 左侧类型必须与 rhs 结果类型一致 |
| 算术 `type` 参数 | 必须与 lhs、rhs 操作数类型一致 |
| 操作数同型 | 两 `Operand` 解析后类型相同 |
| 操作数 Variable | 已定义且类型匹配 |
| Literal 范围 | 落在上下文 `IntType` 可表示范围内 |
| overflow 合法性 | `div`/`mod` 不得出现 `Overflow` 模式 |
| cast 源 | 必须为已定义变量；`CastRhs.source` 非 Literal |
...
以下错误在 v0.7 **均可静态检查**，Analyzer 应尽可能在运行前报出：

- 未定义变量、重复定义、类型错误、字面量范围错误、溢出模式错误

以下在 v0.7 **依赖运行期操作数值**，Analyzer **无法**完全静态判定：
...
execute(program: TypedProgram):
    ...
    for stmt in program.statements:
        match stmt:
            VarDef → eval_rhs → slots[slot] ← value
            Assign → eval_rhs → slots[slot] ← value
    // 正常结束：无 HALT 指令，执行完最后一条即结束
```

**注意**：`VarDef` 在运行时再次执行初始化（与语义「定义并赋值」一致）；Pass 1 已分配槽位，执行时按语句顺序写入。
...
- 与上下文类型结合时，按 §4.3 检查上界

### 9.3 中间计算

| 场景 | 实现要求 |
|------|----------|
| 有符号 strict 加减 | 可在 `int64` 范围检测是否超出 `T` |
| 有符号 overflow 乘 | **必须**使用宽整数（如 `int128`/`uint128` 或等价）先算积，再 mod 2^n 后重解释 |
| 无符号运算 | 宽整数或 `uint64` 内 mod 2^n |
| cast overflow | 按 §7.4 位模式算法，在 `bits` 上操作 |
...
| div | 向零截断；除零 → 错误 | 同 strict（div 无 overflow 模式） |
...
**overflow 模式与无符号**：`add(T, overflow, …)` 对有符号 T 启用回绕；对无符号 T 与三参数形式行为相同。
...
| Analyzer | 标准 §10 中可静态确定的错误 + I/O 语句类型检查 | 否 |
| Executor | 除零、整数溢出、转换溢出、I/O 输入失败 | 否（已部分执行） |

语法错误不在语言标准 §10 枚举内，TC-VM 扩展为 **语法错误**，建议单独错误码。
...
ErrorKind ::=
    ...
    | OverflowModeError
    | CastOverflow
    | IOError                  // 运行时 I/O 错误（read 非法输入、EOF 等）
```

### 11.4 诊断策略

- **静态错误**：报告第一条致命错误即停止（v1.0）；后续可扩展为 collect-all
- **运行时错误**：报告错误时 PC 对应当前 `StatementRecord.line`
- 消息中应包含：错误类型、行号、涉及变量名/类型（若适用）
...
```text
src/vm/
├── runtime/            # TcValue、IntType、ErrorKind、Diagnostic、Semantics
├── lexer/              # Token、词法规则（含 write/writeln/read 关键字）
├── parser/             # StatementRecord 构造（含 write/writeln/read 解析）
├── analyzer/           # SymbolTable、类型检查（含 I/O 语句检查）
├── executor/           # SlotStore、exec_arith、exec_cast、exec_write、exec_read
├── driver/             # run、run_file、CLI
└── CMakeLists.txt      # tc-vm 可执行文件 + check-vm 测试目标
```

各模块目录内放置对应的 `.c` 与 `tc_*.h` 头文件；公共类型与诊断定义位于 `runtime/`。

### 12.2 核心接口（伪代码）

```text
// lexer
tokenize_line(line: string, line_no: int) -> Result<Token[], Diagnostic>

// parser
parse_statement(tokens, line_no) -> Result<StatementRecord, Diagnostic>

// analyzer
analyze(stmts: StatementRecord[]) -> Result<TypedProgram, Diagnostic>

// executor
execute(program: TypedProgram) -> Result<void, Diagnostic>
...
var c: int8 = cast(int8, overflow, sum)
```
...
| 3 | 4 | VarDef c, int8, Cast overflow from sum | slot 3 |
...
| 3 | c ← cast int8 overflow(30)=30 | c=30 |

源 `cast(int8, overflow, sum)` 中 sum=30 在 int8 范围内，overflow 与 strict 结果相同。
...
| 层级 | 对象 | 方法 |
|------|------|------|
| 单元 | `exec_arith`、`exec_cast`、`TcValue` | 表驱动，对照 §6、§7 边界 |
| 组件 | Lexer、Parser、Analyzer | 单条/多条语句快照 |
| 集成 | `run(source)` | 语言标准 §9 全文示例 |
| 回归 | 错误用例 | 每条 §10 错误至少 1 个负例 |

### 14.2 Conformance 目录

```text
tests/
├── valid/           # 正例：执行成功 + 可选期望槽位快照
├── errors/static/   # Analyzer 阶段失败
└── errors/runtime/  # Executor 阶段失败
```

当前用例由 `scripts/vm/run_tests.sh` 驱动；AOT 实现后应复用同一语义用例集（`scripts/aot/run_tests.sh` 预留）。

### 14.3 当前回归用例（v1.1）

| 文件 | 预期 |
|------|------|
| `tests/valid/example.tc` | 执行成功 |
| `tests/valid/signed_overflow.tc` | 有符号 overflow 回绕 |
| `tests/valid/uint8_wrap.tc` | 无符号回绕 |
| `tests/errors/runtime/signed_strict_overflow.tc` | 整数溢出错误 |
| `tests/errors/runtime/div_zero.tc` | 除零错误 |
| `tests/errors/static/duplicate_def.tc` | 重复定义错误 |
| `tests/errors/static/literal_range.tc` | 字面量范围错误 |
| `tests/errors/static/overflow_mode.tc` | 溢出模式错误 |

此外，`scripts/vm/run_tests.sh` 包含 3 个 REPL 回归测试：

| 测试场景 | 输入 | 预期输出 |
|----------|------|----------|
| REPL 跨行变量 | 定义 a/b/sum 并输出 `writeln` | `30` |
| REPL 重复定义错误 | 两次 `var` 同名变量 | 诊断含 `duplicate definition` |
| REPL session reset | `var` 后 `:reset` 再 `:vars` | `(no variables)` |

### 14.4 必测边界

- 有符号 strict 溢出：`add(int8, 127, 1)`
- 有符号 overflow：`add(int8, overflow, 127, 1)` → -128
- 无符号回绕：`add(uint8, 250, 10)` → 4
- div/mod 向零截断与 C 一致用例（§6.2 示例）
- strict / overflow cast 对照（§7.2 示例）
- `div`/`mod` 带 `overflow` → 静态溢出模式错误
...
| 空行、文件尾 | 未规定 | §4 实现约定 |
| 语法错误 | 未枚举 | §11.1 VM 扩展 |
| 静态 vs 运行时错误划分 | 未严格区分 | §7.5、§8.4 实现约定 |
...
语言标准 §12 预留特性在 TC-VM 中的预期挂载点。其中 I/O 语句已在 v1.1 中实现：
...
| I/O 语句 | `Write`/`Writeln`/`Read` 语句类型检查 | `exec_write`/`exec_read` | **已实现** |
...
TC-VM 实现三条 I/O 语句的机制，遵循语言标准 §11 定义的语义。
...
TC-VM 提供交互式 REPL 模式，支持逐条输入 TC 语句并立即执行，变量跨行保留。实现文件为 `driver/repl.c`（~300 行），对外接口定义于 `driver/tc_repl.h`。
...
| 零外部依赖 | 仅使用标准 C（stdio）实现，不引入 readline 等第三方库 |
...
| 符号表 | 局部创建，分析后转入 `TcTypedProgram` | 持久化在 `TcReplSession.symbols` |
| 变量槽 | Executor 内部临时分配 | 持久化在 `TcReplSession.slots`，按需 `realloc` |
| 错误处理 | 失败即终止 | 报告错误后继续接受下一条输入 |

增量分析通过调用 `tc_analyze_statement()` 实现，该函数接受单个 `TcStatement` 和当前符号表指针，将新 `VarDef` 的变量插入符号表并做类型检查。若当前行 `Assign` 引用未定义变量，延迟于该时刻报错。
...
REPL 支持 6 个以 `:` 开头的元命令，在 `tc_repl_handle_meta()` 中处理：

| 命令 | 实现 |
|------|------|
| `:quit` / `:exit` / `:q` | 设置 `should_quit = 1`，退出主循环 |
| `:reset` | 调用 `tc_repl_session_reset()` 释放符号表+槽位，重新初始化 |
| `:vars` | 遍历 `session.symbols`，格式化每个变量的类型与当前值输出 |
| `:help` | 打印内置命令帮助文本 |

元命令不经过词法/语法/分析流水线，在输入读取后直接检查。
...
| 静态分析错误 | 同上；若 `VarDef` 类型检查失败，变量**不**加入符号表 |
| 运行时错误 | 打印诊断，语句的副作用（写槽位）不发生，已有状态保留 |
...
| `n = add(int32, overflow, a, b)` | `ArithRhs(..., Overflow, ...)` |
| `n = cast(int8, x)` | `Assign(line, "n", CastRhs(int8, Strict, "x"))` |
| `n = cast(int8, overflow, x)` | `CastRhs(int8, Overflow, "x")` |
