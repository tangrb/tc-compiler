...
> **当前版本**：TC-VM v0.0.42
>
> **目标语言规范（唯一权威）**：[TC 0.0.42](./TC语言标准设计说明书-0.0.42.md)
>
> **文档职责**：本页准确描述 0.0.42 可执行文件行为与兼容边界。
>
> **内部架构**：[TC-VM 详细设计说明书](./TC-VM详细设计说明书-0.0.42.md)
...
7. [性能计时](#7-性能计时)
8. [当前示例](#8-当前示例)
9. [0.0.42 迁移与兼容边界](#9-0041-迁移与兼容边界)
10. [测试脚本约定](#10-测试脚本约定)
...
`tc-vm` 是 TC 源文件的直接执行引擎。0.0.42 提供：
...
0.0.42 没有活跃 `TcWarningKind`。`TcWarningList` 仍在内部/API 结构中，但正常编译不产生 warning 行。
...
以下示例适用于 v0.0.42。每个源文件须以 `#program` 或 `#lib` 开头（[语言标准 §4](./TC语言标准设计说明书-0.0.42.md)）；未写模式指令为语法错误。
...
*当前命令行为以 0.0.42 源码与本页为准，语言规则以 [TC 0.0.42 标准](./TC语言标准设计说明书-0.0.42.md) 为准。*
