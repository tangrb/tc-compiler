...
| 观察-④ | `Self.` 限定名作常量/整值来源有两层缺陷（初记仅以 struct 例子表述，实际**与类型无关**）：① **常量求值** `tc_eval_const_rhs` 缺 `TC_RHS_SELF_MEMBER` 分支 → 函数内 `let x: int32 = Self.K` 与 `let p: Pair = Self.spc` 两后端均报 `ConstantExpressionError: invalid constant expression`（`static let M = Self.K` 因走 `tc_eval_one_static_let` 专用分支而正常）；② **AOT 常量聚合值发射**把分析期堆指针写进生成 C → `var p: Pair = Self.spc` VM `17` / AOT `0`、`return Self.spc` AOT 段错误、`let b = a` 后再 `var p = b` AOT 段错误 | 库内 `Pair` / `memblock` ＋ `static let` 最小例 | **已闭合（本提交）**：详见下「续修：`Self.` 常量来源与常量聚合值深拷贝」 |
| 观察-⑤ | AOT：`.count` 作 RHS 且所指 memblock 为**常量绑定 + 限定名**时 `tc-aot: code generation failed`（rc=1，`--check` 两后端均通过）；VM 正常。**根因更正（详查后）**：不是「FIELD_READ 缺 `is_memblock_count`」——RHS 位置的 `.count` 由解析器归为 `TC_RHS_MEMBLOCK_COUNT`（`tc_parser_rhs.c:39`），`is_memblock_count` 只出现在 operand 位置且 AOT 已处理；真实失败点是 `tc_aot_emit_rhs.c` 的 `TC_RHS_MEMBLOCK_COUNT` 分支：常量绑定槽位为 −1，而「折叠声明 count」那条路径依赖按 `memblock_name`（= `"Self.M"` / `"Mod.C"`）查符号，符号表以裸名存放 → `sym == NULL` → `return -1`；`binding.type` 已带 memblock 类型却未被使用 | `var n: usize = Self.M.count` / `n = Self.M.count` / `var n: usize = MbLib.C.count`（均 AOT 失败）；对照：`Self.V.count`（static var）、`mb.count`（局部 var）、顶层 `let M` 的 `M.count`（裸名 const）、`add(usize, Self.M.count, 1)`（operand）、`let n: usize = Self.M.count`（常量上下文）均两后端一致 | **已闭合（本提交）**：详见下「续修：`.count` 作 RHS 的常量绑定路径」 |
| 观察-⑥ | `<模块>.<名> = rhs` 整绑定赋值目标未解析：`MemberLib.W = 9`、`MemberLib.K = 9` 均报 `UndefinedVariable: undefined variable 'MemberLib'`。**根因（详查）**：不是解析函数缺限定名支持（读 / 字段读 / 字段写 / `read` 目标 / `memblock_store`·`memblock_copy` 目标均已支持），而是**该语句形态根本没有走到整绑定赋值机制**——`Self.<名>` 在解析器有专用分支直接产出 `TC_STMT_ASSIGN{name="Self.W"}`，而 `<模块>.<名>` 走「标识符 + 单点」路径被无条件塑造成 `TC_STMT_FIELD_ASSIGN{base="MemberLib", fields=["W"]}`，字段赋值再拿前缀当基对象解析 → `undefined variable 'MemberLib'`（附录 A `assignment` 明确允许 `imported_member_name` 作目标） | `MemberLib.W = 9` / `MemberLib.K = 9` / `MemberLib.HiddenVar = 3` / `MemberLib.W = true`（分别应为：接受、`CONSTANT_ASSIGNMENT`、`PRIVATE_MEMBER_ACCESS`、`TYPE_MISMATCH`）；对照 `MemberLib.spv.x = 7`（字段写）、`Self.W = 3`（本模块整绑定写）、`read(int32, MemberLib.W)`、`memblock_store(..., MbLib.M, ...)` 均已通过 | **已闭合（本提交）**：详见下「续修：`<模块>.<名> = rhs` 整绑定赋值目标」 |
...
| `Self.<名>` 作用域收口（同一要求的第二面） | 用户追问「private 只能被模块内访问」后补测发现：`Self.<名>` 走的是**裸名查全局符号表**（`tc_symbol_table_find(global, member)`），故被导入模块 A 内写 `Self.Hidden` / `Self.K` / `Self.spc.x` 会命中另一模块的同名成员（含 private）。① 新增 `tc_resolve_self_member(member, global)`（`tc_analyzer_pass2.c`）：先用当前模块的成员索引 `g_name_scope_members` 确认该名确属本模块，再取符号；`tc_find_named_binding` 的 `Self.` 分支、`tc_struct_resolve_base` 的 `Self.` 基址分支、`tc_type_check_rhs` 的 `TC_RHS_SELF_MEMBER` 三处统一改走该助手。② static let **提前求值**在 Pass2 之前、无作用域上下文，故 `tc_eval_one_static_let` 增 `module_name` 形参并按符号模块标记比对（同时避免取到错误模块的常量值）。③ 界定：`tc_const_eval.c` 的 `tc_try_eval_self_member_bool`（静态布尔折叠）与 `tc_cfg.c` 的按名读节点仍按裸名解析，但 Pass2 的类型/操作数检查先于接受判定，跨模块 `Self.` 已在上述三处被拒，不会使非法程序通过 | 新增 3 条单错误 `#lib` 负例（`#lib` 可作入口）：`tests/modules/SelfForeignPrivLib.tc`（`Self.Hidden` 外来 private）、`SelfForeignPubLib.tc`（`static let X = Self.K`，同时覆盖提前求值路径）、`SelfForeignFieldLib.tc`（`Self.spc.x` 外来 struct 成员）；VM 3×`run_expect_check_fail`（消息＋`UndefinedVariable`）、AOT 3×`run_check_fail`；test-map 回填 **1180 VM / 552 AOT** | 三条负例两后端均报 `undefined variable '<成员名>'`（rc=1 同阶段；`Self` 指本模块，本模块无该名 → 按未定义变量报告，而非 `PRIVATE_MEMBER_ACCESS`——后者专用于 `<模块>.<名>` 限定形态）；对照正例 `MemberLib.hidden_self()`（本模块 `Self.Hidden`＋`Self.HiddenVar` → `106`）与既有 `self_*`／`import_self_call` 全部通过；VM 全量 1178 项 0 失败；全量三层与 5 项门禁通过 |

### 续修：`Self.` 常量来源与常量聚合值深拷贝（观察-④ 闭合）

> 触发：用户要求对观察-④ 详述。详查把原记录（以 struct 例子表述）拆成**两层、且第一层与类型无关**的缺陷；语言标准 §5.2.1 的原子表达式含「经 `Self.` 解析到的 `static let`」（无类型限制），故两层都属实现未跟上规范口径。

| 项 | 实现 | 语料与门禁 | 实测 |
| --- | ---- | ---------- | ---- |
| 观察-④-A：常量求值缺 `TC_RHS_SELF_MEMBER` | `tc_const_eval.c` 的 `tc_eval_const_rhs`（`let` 初始化器常量求值入口）新增 `TC_RHS_SELF_MEMBER` 分支：`tc_resolve_self_member` 取符号（含 §4.3/§4.4 本模块归属判定）→ 校验 `TC_SYM_CONSTANT` / `has_const_value`（含挂起 CT 诊断的派生失败口径）→ 类型闭合 → `*out = symbol->const_value`，与相邻 `TC_RHS_CONST_REF` 分支完全同口径（struct/memblock 的堆块别名判定与所有权仍由 `tc_resolve_const_value` 处理）。此前该 kind 落到函数末尾兜底「invalid constant expression」；`static let M = Self.K` 另走 `tc_eval_one_static_let` 专用分支，故同一 RHS 形态在两个常量上下文行为不一致 | `tests/valid/SelfConstAggLib.tc` 的 `const_scalar`（`let x: int32 = Self.K`，标量——原记录未覆盖）与 `const_struct`（`let p: Pair = Self.spc`）；入口 `tests/valid/self_const_agg.tc`；VM `run_expect_stdout` ＋ `run_expect_check_ok`、AOT `run_diff_test` ＋ `run_check_ok` | 修复前：`let x: int32 = Self.K` 与 `let p: Pair = Self.spc` 两后端均 `ConstantExpressionError`；修复后：`41` / `17`（两后端一致）。`static let B = Self.A`（`7`）与运行时 `var x = Self.K`（`41`）行为不变 |
| 观察-④-B：AOT 常量聚合值发射分析期堆指针 | `tc_aot_codegen.c` 新增 `tc_aot_emit_const_struct_expr` / `tc_aot_emit_const_struct_assign`（内联常量字节 → 运行期 `tc_aot_struct_extract` 重新分配，值语义 §3.9.4；与该文件既有的 const memblock 发射同口径），并把这些**常量**分支接上：① `tc_aot_emit_rhs` 的 `TC_RHS_CONST_REF`（binding 已解析路径与按名回退路径）；② `TC_RHS_SELF_MEMBER` 的常量分支（struct 用新助手、memblock 用 `tc_aot_emit_const_memblock_expr`）；③ `tc_aot_emit_operand_assign` 新增常量 struct/memblock 分支（覆盖 `return Self.spc` 等经 `TcOperand` 的赋值路径），`tc_aot_emit_operand_expr` 的常量 struct/memblock 表达式位同步；④ `tc_aot_emit_rhs` 的 FIELD_READ 0 字段分支改用同一助手（既有-1 的实现合并，避免多份复制） | `tests/valid/const_struct_copy.tc`（`let b = a` → `var p = b`，即 `CONST_REF` 常量 struct 路径）、`tests/valid/const_memblock_copy.tc`（常量 memblock 路径）、`SelfConstAggLib.tc` 的 `var_struct` / `ret_struct` / `const_memblock` / `var_memblock` / `alias_check`；VM `run_expect_stdout` ＋ `run_expect_check_ok`、AOT `run_diff_test` ＋ `run_check_ok`；test-map 回填 **1186 VM / 558 AOT** | 修复前三处发散：`var p: Pair = Self.spc` VM `17` / AOT `0`；`return Self.spc` VM `17` / AOT **段错误**；`let b = a` 后 `var p = b` VM `7` / AOT **段错误**（`CONST_REF` 同一缺陷）。修复后六条探针与三个新语料两后端逐行一致：`self_const_agg` → `41 17 17 17 17 6 1 8`、`const_struct_copy` → `7`、`const_memblock_copy` → `6`；生成 C 中常量 struct 形如 `tc_aot_struct_extract((uint64_t)(uintptr_t)&(const uint8_t[]){8,0,0,0,9,0,0,0}, 0, 8, …)`，不再出现 `slots[N] = 0x<宿主地址>`；全量三层与 5 项门禁通过 |

### 续修：`.count` 作 RHS 的常量绑定路径（观察-⑤ 闭合）

> 触发：用户要求对观察-⑤ 详查。详查把台账原记录的根因（「语句级 FIELD_READ 缺 `is_memblock_count`」）**更正**为 `TC_RHS_MEMBLOCK_COUNT` 分支的常量绑定路径缺失，并给出隔离实验（裸名 const 通过、限定名 const 失败 → 定位到「按限定名查符号」）。

| 项 | 实现 | 语料与门禁 | 实测 |
| --- | ---- | ---------- | ---- |
| 观察-⑤：AOT `.count` 常量绑定路径 | `tc_aot_emit_rhs.c` 的 `TC_RHS_MEMBLOCK_COUNT` 分支新增一条路径：`binding.resolved && binding.slot < 0 && binding.type->tag == TC_MEMBLOCK` 时直接折叠 `tc_type_memblock_count(binding.type)`；已有的「符号表槽位读头部」「符号表声明 count」两条按名路径保留作回退。原因：RHS 位置的 `.count` 由解析器归为 `TC_RHS_MEMBLOCK_COUNT`（`tc_parser_rhs.c:39-49`，**不是** FIELD_READ），而符号表以**裸成员名**存放，`memblock_name` 为 `"Self.M"` / `"Mod.C"` 时按名查符号失败；常量绑定槽位又是 −1（`tc_resolved_binding_set` 对非 `TC_SYM_VARIABLE` 置 −1），于是三条旧路径全部落空 → `return -1`。（`is_memblock_count` 标志只由 operand 位置的 `tc_struct_check_field_access` 产生，AOT operand 发射器早已处理，故原记录定位不准确。） | 新增 `tests/valid/MbCountLib.tc`（public const + public static var memblock，函数覆盖 `Self.C.count` 语句级/赋值 RHS、`Self.W.count`、operand、常量上下文、局部裸名、**形参基址**）+ 入口 `tests/valid/memblock_count_rhs.tc`（另含入口侧导入限定名 const/static var、operand 与顶层 `let` 裸名对照）；VM `run_expect_stdout` ＋ `run_expect_check_ok`、AOT `run_diff_test` ＋ `run_check_ok`；test-map 回填 **1188 VM / 560 AOT** | 修复前 4 例 AOT 硬失败（`var n = Self.M.count`、`n = Self.M.count`、`var n = MbLib.C.count`、`n = MbLib.C.count`，均 `tc-aot: code generation failed`，而 `--check` 两后端 rc=0、VM 输出 `2`）。修复后 13 条探针 + 新语料两后端逐行一致：`memblock_count_rhs` → `2 2 2 3 2 2 2 2 3 2 2`；生成 C 中常量基址折叠为 `slots[1] = 2ULL;`，运行期基址仍读头部 `tc_aot_memblock_get_count(slots[0])`（行为不变）。边界确认：附录 A `memblock_count_access` 的基址**限于绑定**（含形参），故 `h.mb.count`（结构体字段链）两后端一致静态拒绝 `TypeMismatch: field read requires struct base`，非分歧。全量三层与 5 项门禁通过 |

### 续修：`<模块>.<名> = rhs` 整绑定赋值目标（观察-⑥ 闭合）

> 触发：用户要求先出针对性分析再修。分析与实测把根因从「赋值目标解析路径未支持限定名」更正为「该语句形态未进入整绑定赋值机制」（`Self.<名>` 有解析器专用分支故正常；`<模块>.<名>` 被塑造成单字段字段赋值）。按**修法 A**（分析器内重分类为 `TC_STMT_ASSIGN`）实施，不触碰解析器与后端。
...
当前状态：**无待办条目**（`§5` 九项 ☑、观察-③～⑥ ☑）；规模 **1197 VM / 568 AOT（注册） / 634 AOT（执行） / ~3323 unit**；全量三层与 5 项同步门禁通过；全部提交**未推送**。

## 标准 owner 裁决记录（A-1～A-4 与 B-41/B-61/B-63 全部裁决并落地）

> 七项裁决与落地明细见上「阶段 3 逐条记录」，**本轮无待裁决项**。
> 裁决摘要：A-1 RHS 不属于 `operand`（禁嵌套）；A-2 `TC_CE_LITERAL_OUT_OF_RANGE` 阶段列 `LT / SEM`；A-3 编译期可确定负下标/负 `length` 报静态码；A-4 `nullptr` 不参与 `bitcast`；B-41 `else`/`end` 不对齐一律 `ELSE_END`；B-61 专用码优先于 `CONDITION_TYPE`；B-63 顶层行缩进非法。
...
| 90 | 观察-③ 限定名成员可见性（`<模块>.<private>`） | `9a0ca86 fix(0.0.44-§4.4): reject private members in qualified name resolution` |
