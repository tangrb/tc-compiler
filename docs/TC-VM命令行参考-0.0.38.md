...
> **当前版本**：TC-VM v0.0.38
>
> **目标语言规范（唯一权威）**：[TC 0.0.38](./TC语言标准设计说明书-0.0.38.md)
>
> **文档职责**：本页准确描述 0.0.38 可执行文件行为与兼容边界。
>
> **内部架构**：[TC-VM 详细设计说明书](./TC-VM详细设计说明书-0.0.38.md)
...
7. [性能计时](#7-性能计时)
8. [当前示例](#8-当前示例)
9. [0.0.38 迁移与兼容边界](#9-0035-迁移与兼容边界)
10. [测试脚本约定](#10-测试脚本约定)
...
`tc-vm` 是 TC 源文件的直接执行引擎。0.0.38 提供：
...
下表描述 0.0.38 的 `TcErrorKind` 完整映射。Language 诊断默认打印 message，不直接打印这些名称。完整错误码清单、阶段归属与触发条件见编译器标准 §11.4。
...
0.0.38 没有活跃 `TcWarningKind`。`TcWarningList` 仍在内部/API 结构中，但正常编译不产生 warning 行。
...
*当前命令行为以 0.0.38 源码与本页为准，语言规则以 [TC 0.0.38 标准](./TC语言标准设计说明书-0.0.38.md) 为准。*
