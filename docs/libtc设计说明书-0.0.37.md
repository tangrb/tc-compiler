...
>
> **当前实现基线**：libtc / TC-VM v0.0.37
>
> **状态**：承载 TC 0.0.37 的 libtc 架构设计，涵盖模块系统、函数、memblock、ptr、struct 与完整 13 阶段编译管线。
>
> **调用者速查**：[libtc API](./libtc-api-0.0.37.md)
...
| 维度 | 版本 | 说明 |
| ---- | ---- | ---- |
| 目标语言规范 | 0.0.37 | libtc 最终必须实现的接受集、结果和诊断阶段 |
| 编译器规范 | 0.0.37 | 确定的 13 阶段编译管线、错误码与检查顺序 |
| 本文 | 0.0.37 设计 | 面向当前语言能力的实现设计 |
...
0.0.37 新增 `tc_set_module_search_paths` 用于多文件模块搜索；`tc_compile_source` 新增 `name` 参数用于诊断和模块解析。`tc_run_typed` 重命名为 `tc_run_program` 以反映其涵盖模块静态初始化的语义。

### 2.2 源兼容与二进制兼容

函数签名保持源兼容，但 `TcTypedProgram`、`TcDiagnostic` 和错误枚举是公开可见 C 结构；0.0.37 大幅扩展这些结构（函数签名、模块元数据、memblock/ptr/struct 类型参数）。嵌入方必须重新编译。本文不承诺跨版本的二进制 ABI 兼容。
...
`tc_statement_free` 必须按 kind 释放所有新增的 0.0.37 payload：函数定义的 body 和形参列表；funcall 的目标名和命名实参；return 的操作数；struct 定义的字段列表；memblock 构造器的元素列表；ptr_* 指令的操作数；import 声明；以及字段赋值/读取的字段名。每个新增 kind 必须同步 free 分发；未知 kind 在调试构建中断言，不能静默泄漏。
...
0.0.37 编译器标准 §11.4 是 `TcErrorKind` 的权威清单。错误码按阶段分为：
...
| `memblock<T,N>` 类型 | count 参数语法 | N 记录、类型等价不含 N、N 比较 |
| `struct` 类型 | 字段声明、@padding | 构造器全字段验证、双层可变性、布局 |
...
*语言合法性与错误种类以 [TC 语言标准 0.0.37](./TC语言标准设计说明书-0.0.37.md) 与 [TC 编译器标准 0.0.37](./TC编译器标准设计说明书-0.0.37.md) 为准；当前可调用行为以 [libtc API](./libtc-api-0.0.37.md) 为准。*
