...
>
> **当前实现基线**：TC-AOT v0.0.39（`TC_AOT_VERSION`）
>
> **状态**：0.0.39 代码生成架构设计，涵盖模块系统、函数、memblock、ptr、struct 与完整 C99 代码生成。
>
> **上游契约**：[TC-VM 详细设计说明书](./TC-VM详细设计说明书-0.0.39.md) 的 typed program、完整 CFG 与共享运行时语义
...
| 维度 | 版本 | 状态 |
| ---- | ---- | ---- |
| 目标语言 | TC 0.0.39 | 规范已确定 |
| 编译器规范 | TC 编译器 0.0.39 | 13 阶段管线已确定 |
...
若 host 编译器或未来资源清理要求不适合原生 C `while`，允许改用唯一内部标签实现同一 CFG。两种策略必须通过相同差分用例。
...
字段类型检查与 [语言标准 §3.9.1] 一致：值位置（字段类型本身或 `memblock` 元素类型）禁止自引用与前向引用；指针所指位置允许 `ptr<正在定义的本结构体>`（指针自引用）。`ptr<S>` 字段宽度恒为指针宽度，不阻塞 `sizeof(S)` 累加。未决结构体名经 `TcType.pending_name` 在注册后解析为 `struct_id`（与 VM 共用 Analyzer）。
