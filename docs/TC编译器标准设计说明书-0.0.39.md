...
本文档定义 TC 编译器为实现 [TC语言标准设计说明书-0.0.39.md](./TC语言标准设计说明书-0.0.39.md) 所必须遵循的确定性编译阶段、分析算法、诊断优先级与错误码体系。
...
本说明书是 0.0.39 编译器行为的规范性条款：凡影响可观察诊断、错误码、静态接受集或与语言标准交叉引用的算法步骤，均必须按本文实现。VM 与 AOT 共享同一前端分析语义；代码生成见各自实现文档，但不得改变本文规定的静态结果。
...
- **字段类型检查**：字段类型仅可为标量（`scalar_type`）、`memblock<T, N>`（带 `N` 规划）、`ptr<T>`，或结构体类型名。结构体名按 [语言标准 §3.9.1] 的位置规则分流：值位置（字段类型本身或 `memblock` 元素类型）仅允许本模块源序更早已成功定义完毕的结构体，禁止当前结构体自身与前向引用；指针所指位置允许完整类型或**正在定义的本结构体**（指针自引用），仍禁止其它未完成定义的结构体名。值位置自引用 → `TC_CE_STRUCT_VALUE_SELF_REF`；前向引用或未定义结构体 → `TC_CE_UNDEFINED_STRUCT`。`void` 字段类型不合法。
...
- 分析结果是 0.0.39 的强制语义，不允许实现通过更弱或更强的默认规则改变合法程序集合。
...
`TC_CE_VAR_MISSING_INIT` 不得降级为通用语法错误；`TC_CE_UNINITIALIZED_VARIABLE` 不得替代名称解析错误。0.0.39 不定义 `TC_CE_GOTO_SKIPS_VAR_INIT`，因为未初始化是跨全部控制流种类的通用性质，不应绑定到单一语句。
...
0.0.39 不定义 `TC_CE_GOTO_SKIPS_VAR_INIT`、常量循环依赖专用错误、跨块引用专用错误或重复的 overflow-mode 错误：它们分别由确定初始化、源序可见性/名称解析，以及"EBNF 先行、模式语义随后"的统一规则处理。语言正文对 `funcall` 实参缺失/重复/乱序等仅要求"静态错误"时，本说明书 §8.2 / §11.4.2 给出确定性专用码映射，实现必须采用这些码，不得合并为笼统 `TC_CE_SYNTAX` 或 `TC_CE_TYPE_MISMATCH`（除非附录 A 已在语法阶段拒绝）。
...
下列各表是 0.0.39 完整、权威的标准错误码对照。语言错误码使用 `TC_CE_*`（编译期/静态）与 `TC_RE_*`（运行时）前缀，另含实现资源失败码 `TC_ERR_OUT_OF_MEMORY`。语言错误码、打印名与诊断类别一一对应，不得合并、别名化或以其他错误码代替；布尔 `xor` 与浮点 `mod` 复用既有错误码。本表涵盖语言附录 B 全部码，以及语言正文仅写"静态错误"、由本说明书确定性映射的函数/语法专用码。
...
| `TC_CE_UNDEFINED_STRUCT` | `UndefinedStruct` | 未定义结构体 | 静态 | 类型位置或构造器名无法解析为已定义（且可见）的结构体类型；字段类型前向引用其它尚未完成定义的结构体 |
