...
| 文档 | 用途 |
| ---- | ---- |
| [TC 语言标准设计说明书-0.0.45.md](TC语言标准设计说明书-0.0.45.md) | **0.0.45 语言标准草案**：`ref<T>`（托管引用）与 `addr<T>`（线性地址）双空间、公共 IR 文本形态定位 |
| [TC 编译器标准设计说明书-0.0.45.md](TC编译器标准设计说明书-0.0.45.md) | **0.0.45 编译器标准草案**：与语言标准对齐的检查规则、类型化中间形式与降级契约（§1.4）、局部可判定（§1.5）、调用深度上界（§8.10） |
...
当前核心版本：**v0.0.44**（语言规范与现行设计文档为 **0.0.44**）。0.0.45 草案见上表，尚未并入语言标准。
