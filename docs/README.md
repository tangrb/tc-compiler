...
>
> **冻结评估未通过（2026-09-26）**：两份 0.0.45 设计文档保持**冻结候选**，不构成冻结规范（原 `spec-0.0.45` 标签已撤销）。待闭合三处缺口：托管引用的生命周期与悬垂引用语义、地址算术值语义与目标字长、语法受限类型位置的诊断阶段。冻结后仍仅接受勘误，任何语义变更必须升版；勘误与跨版本差异统一记录于 `CHANGELOG.md`，设计文档正文不承载修订历史。
