...
> **后续变更注记（2026-08-29）**：本报告审查范围内的下列文档已删除/合并——`设计实现合规审查报告-0.0.42.md`（已删除）、`TC-0.0.41-结构体字段operand修复计划.md`（已删除，其修复已落地）、`libtc-api-0.0.42.md`（已并入 `libtc设计说明书-0.0.44.md` §15）、`TC-0.0.41-开发计划.md`（已删除，属老版本开发计划，历史见发布版本）。§6 表格保留原审计结论并标注现状。
>
...
降级项的实现侧说明见 [TC-AOT 详细设计说明书](./TC-AOT详细设计说明书-0.0.44.md) §19。任务对照与阶段状态见[修复计划](./TC-0.0.41-语言标准符合性修复计划.md) §10。`examples/` 已移出版本库，不作为发布门禁路径。
