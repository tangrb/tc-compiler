...
> **规范基线（唯一权威）**：[TC 语言标准 0.0.41](./TC语言标准设计说明书-0.0.42.md)（§1–§11 + 附录 A/B，3371 行）
...
> **后续变更注记（2026-08-29）**：本报告审查范围内的下列文档已删除/合并——`设计实现合规审查报告-0.0.42.md`（已删除）、`TC-0.0.41-结构体字段operand修复计划.md`（已删除，其修复已落地）、`libtc-api-0.0.42.md`（已并入 `libtc设计说明书-0.0.42.md` §15）、`TC-0.0.41-开发计划.md`（已删除，属老版本开发计划，历史见发布版本）。§6 表格保留原审计结论并标注现状。
>
...
- **规范权威**：`docs/TC语言标准设计说明书-0.0.42.md`。正文决定静态与执行语义，附录 A 决定语法接受集，附录 B 为错误码速查（§1.3）。
...
降级项的实现侧说明见 [TC-AOT 详细设计说明书](./TC-AOT详细设计说明书-0.0.42.md) §19。任务对照与阶段状态见[修复计划](./TC-0.0.41-语言标准符合性修复计划.md) §10。`examples/` 已移出版本库，不作为发布门禁路径。
