...
19. [已知可移植性债务](#19-已知可移植性债务不在-0041-强制范围)
...
本文按 0.0.44 规范口径书写（**文档先行**）。AOT 不自行实现前端，静态项由**共享 Analyzer**（见 [TC-VM 详细设计说明书 §1.5](./TC-VM详细设计说明书-0.0.44.md)）保证；下表列出与 AOT 相关的落地差异，同步项同时登记于《TC 语言标准一致性勘误清单》§9。

| 条目 | 变更记录 | 影响面 | 状态 |
| ---- | -------- | ------ | ---- |
| `var` 缺初始化器（第 3 阶段）、结构体名三类冲突、前导零与实参形态码 | D4／D7／D17 | 输入契约（§3.1 要求「已通过阶段 3」） | **已同步**（共享 Analyzer；AOT 只消费其结果） |
| `read` 标准输入读取失败报 `TC_RE_IO` | D31 | 共享 `tc_io`（§12／§14） | **已同步** |
| 浮点语义：IEEE 754-2019、roundTiesToEven、禁止 FMA／FTZ／DAZ、`mod` 商向零截断 | — | 共享纯语义 core（§12） | **已同步**（VM/AOT 共用 core） |
| 指针 `cast` **不附加等宽条件** | D14 | 共享 Analyzer | **待同步（W-6）**：`tests/errors/static/ptr_cast_width.tc` 仍断言等宽拒绝（见 VM 详设 §1.5） |
| `bitcast(ptr ↔ 浮点)` 改为拒绝 | D10 | 共享 Analyzer（§12 生成路径不受影响） | **待核对（W-5）**：语料未覆盖 |
| `ptr_size` 只可整条充当 `const_rhs` | D29 | 共享 Analyzer（§9.3 内联） | **待核对（W-13）** |
| `const_rhs` 中嵌套调用报 `TC_CE_SYNTAX` | D34 | 共享 Analyzer | **待同步（W-26）** |
| CT 类诊断须晚于全部 SEM 类诊断报告 | D19 | 共享 Analyzer 阶段顺序 | **待同步（W-27）** |
| `ptr_load` 结果类型为 `bool` 时规范化；所有 `bool` 写路径归一化 | D16 | 共享 core 与 §4 槽表示 | **待核对（W-9）** |
...
内部 slot 以 `uint64_t` 保存位模式。若源和目标都是规范化的等位宽 slot 表示，bitcast 可以直接复制并按位宽掩码。**`ptr` ↔ 浮点的 bitcast 为静态拒绝**（语言标准 §6.6.6／§3.10.9）：AOT 不得为 `bitcast(float64, ptr_val)` 一类形态生成任何等价路径，也不得把 `ptr` 值经宿主指针/浮点对象表示中转：
...
10. 差分测试扩展；
11. 全量门禁通过后升版至 0.0.42。

---

## 19. 已知可移植性债务（0.0.42 已清零）

0.0.41 收口时记录的六项债务已在 0.0.42 全部清零（原清零计划文档已作为历史过程文档移出仓库，如需追溯见发布标签 `v0.0.42`）：
...
**剩余实现说明**：本实现维持 64-bit-only（memblock 头部宽 = `sizeof_bits(usize)` = 64 位，含编译期断言）；32 位目标 / 大端主机支持不在 0.0.44 范围，但 A1/A2 的位级约定已保证任意字节序主机的数值一致性。
