...
| §3 | [语言标准 §3] 类型系统 | **§3.0 内部类型事实源（`const TcType*` / `TcTypeTag` 投影）**；**§3.0.1 `sizeof_bits` 宽度速查**；整数类型有符号/无符号编译器约束（[语言标准 §3.2]、[语言标准 §3.5]）；`bool` 抽象宽度与规范位模式验证（[语言标准 §3.4]）；字面量检查实现要点（[语言标准 §3.6]）；类型转换通用框架编译器验证（含 ptr cast/bitcast，[语言标准 §3.7]）；`memblock<T, N>` 规划个数与类型等价分离、运行时布局计算（[语言标准 §3.8]）；结构体类型编译器约束、构造器验证、字段可变性双层检查、运行时布局与 @padding 验证（[语言标准 §3.9]）；`ptr<T>` 静态约束、指针来源、比较运算与算术验证（[语言标准 §3.10]） |
...
本说明书是 0.0.44 编译器行为的规范性条款：凡影响可观察诊断、错误码、静态接受集或与语言标准交叉引用的算法步骤，均必须按本文实现。VM 与 AOT 共享同一前端分析语义；代码生成见各自实现文档，但不得改变本文规定的静态结果。
...
| 3 | 语法解析；**结构类语法阶段诊断**（`TC_CE_MODULE_LAYER` / `TC_CE_MISSING_VISIBILITY` / `TC_CE_PROGRAM_MODE_MISUSE` / `TC_CE_VAR_MISSING_INIT`）；操作数数量语法恢复 | [语言标准 §1.3]、[语言标准 §5.1.2] 附录 A |
...
4. **模块结构、导入解析与函数签名收集**。本阶段内部按以下子阶段顺序执行，前一子阶段有错时不进入后一子阶段；跨文件诊断的主位置与关联位置规则见 §4.1：
   ...
   4a. **单文件模块结构**：检查五层排序（`TC_CE_MODULE_LAYER`）、`#lib` 成员是否带显式 `public` / `private`（`TC_CE_MISSING_VISIBILITY`，覆盖 `struct` / `func` / `static let` / `static var`；缺可见性前缀亦可在第 3 阶段结构类语法阶段诊断中报告，见附录 A.3）、以及 `#program` 是否误用 `func` / `Self` / `static` / `public` / `private`（`TC_CE_PROGRAM_MODE_MISUSE`，含带可见性前缀的 `struct`）。
   ...
   4c 成功后、进入 4d 之前，编译器注册结构体表（实现步骤，`tc_struct_table_register_program`）：按依赖拓扑登记全部可达模块的 `struct`，条目以 `(定义模块名, 结构体名)` 定界。类型位置与值构造器中的结构体名（裸标识符或 `imported_member_name`）在此解析，规则见 §3.3。未解析为已定义且可见的结构体 → `TC_CE_UNDEFINED_STRUCT`；经导入限定解析到存在但为 `private` 的结构体 → `TC_CE_PRIVATE_MEMBER_ACCESS`（不得改报未定义）。同一模块重复定义 → `TC_CE_DUPLICATE_STRUCT`。
...
**诊断类阶段与处理阶段的对应（0.0.44）**：本节 13 个阶段是**实现步骤**编号，不等同于 [语言标准 §11] 四步规则中的诊断类阶段（LT / SYN / SEM / CT）。对应关系为：第 1–2 阶段 → **LT**；第 3 阶段 → **SYN**；第 4–8、11、12 阶段 → **SEM**；第 9–10 阶段的常量求值与三态判定 → **CT**（其形态与来源类子条件仍属 SEM，见 [语言标准 §5.2.1]）。因此 [语言标准 §11] 的「阶段优先（LT → SYN → SEM → CT）」要求 **CT 类诊断必须晚于全部 SEM 类诊断报告**：实现必须在第 9、10 阶段**挂起**属于 CT 类的诊断（`TC_CE_CONSTANT_OVERFLOW` / `TC_CE_CONSTANT_DIV_ZERO` / `TC_CE_CONSTANT_CAST_OVERFLOW`，以及求值类 `TC_CE_CONSTANT_EXPRESSION`），只使用其**数值结果**供第 10 阶段三态判定与第 11 阶段可达性分析；若第 11 阶段（可达性、确定初始化、函数末尾）或第 12 阶段（调用图环）产生 SEM 诊断，则报告该 SEM 诊断并终止，挂起的 CT 诊断不再报告。仅当全部 SEM 检查均无触发时，才按第 2 条源位置顺序报告挂起的 CT 诊断。未按此顺序实现将违反 [语言标准 §11] 的规范性顺序，并使「首个规范诊断及其错误码」跨实现不一致。
...
函数参数名受保护：任意层级 `var`/`let` 与形参同名 → `TC_CE_DUPLICATE_DEFINITION`（第 6 阶段，主位置为内层标识符，关联形参位置）。全局函数名受保护：同一文件同名函数，或与任何值绑定同名 → `TC_CE_FUNCTION_NAME_CONFLICT`（同名函数与顶层直接绑定在第 5 阶段，嵌套/函数体内在第 6 阶段；主位置为后声明的函数名或值绑定名，关联先前定义）。

附录 A 首先决定 Token 序列能否进入静态语义（[语言标准 §1.3]）。不符合产生式属**语法拒绝**（第 3 阶段）：缺初始化器 → `TC_CE_VAR_MISSING_INIT`，操作数数量错 → `TC_CE_OPERAND_COUNT`，无模式位置的模式关键字 → `TC_CE_SYNTAX`，缩进/块形态 → 对应专用码（`else`/`end` 不对齐 → `TC_CE_INDENT_ELSE_END`），声明/语句分区破坏 → `TC_CE_MODULE_LAYER`；其余用 `TC_CE_SYNTAX`。格式说明符：无法匹配 EBNF → 第 3 阶段 `TC_CE_SYNTAX`；已匹配但控制项非法 → 第 6 阶段 `TC_CE_FORMAT_SPECIFIER`。
...
缺少右括号、顶层逗号结构破损或无法识别内建调用外壳时，使用 `TC_CE_SYNTAX` 而非 `TC_CE_OPERAND_COUNT`。嵌套调用也不属于操作数数量错误：`let` RHS（`const_rhs`）中的嵌套调用与其它不允许嵌套的位置一并在第 3 阶段报 `TC_CE_SYNTAX`（[语言标准 §5.2.1]）。`funcall` 的项是命名实参，不纳入本错误码；其缺失、重复、未知与顺序错误分别使用 §8.2 的专用诊断。`return`、`if` / `while`、变量/常量定义与其他语句形态也不属于内建固定元数调用外壳，沿用各自的语法或专用诊断。
...
| `<模块名>.<成员>` 作普通 RHS 操作数的**整体读取**（标量与 struct；含 `let` 常量上下文）；限定前缀须确为该成员所属模块 | §3.2、§6.7、[语言标准 §4.3、§5.2.1、§6.1.2] | **已同步**：`tc_struct_check_field_access` 按无字段绑定定型，Executor／const-eval／AOT 三端按绑定读取；语料 `import_member_operand.tc`／`import_member_struct.tc`，负例 `import_member_{bad_qual,foreign_member}.tc` |
| `ptr_address(T, Self.<名>／<模块名>.<名>)` 合法（限可写 `static var`） | §3.2、§6.7、[语言标准 §6.8.4] | **已同步**：`tc_parse_ptr_address_rhs` 复用绑定名解析；语料 `import_addr_self.tc`／`import_addr_qual.tc` |
| `ptr_store`／`memcopy_unsafe` 只读判据为**所指外层绑定**（指针绑定自身为 `let` 不构成只读）；常量 `nullptr` 指针的写入归运行期 `TC_RE_NULL_POINTER_DEREFERENCE` | §3.2、§6.7、§11.4.6、[语言标准 §6.8.3、§3.10.2] | **已同步**：`tc_ptr_check_store` 只查 `ptr_target_readonly`，`CONST_REF` 分支只继承来源指针标记；语料 `ptr_store_null_let{,_copy}`／`memcopy_unsafe_null_let`（运行时）、`ptr_store_readonly_copy`（静态传播） |
...
| `Self.<名>` 只解析**本模块**顶层成员，不得命中其它模块的同名成员（含 `private`） | §3.1 第 6 阶段 6d、§4.3、[语言标准 §4.3、§4.4] | **已同步**：`tc_resolve_self_member` 按当前模块成员索引判定归属（static let 提前求值路径按 `program->module_name` 比对）；语料 `tests/modules/SelfForeign{Priv,Pub,Field}Lib.tc`（`UNDEFINED_VARIABLE`），对照正例 `MemberLib.hidden_self()` |
| 经 `Self.` 解析到的 `static let` 可作 `let` 初始化器的常量来源（标量 / struct / memblock 同口径）；常量聚合值拷贝须深拷贝（生成代码不得嵌入分析期堆指针） | §3.2、§5.2.1、§6.7、[语言标准 §5.2.1、§3.9.4、§3.8.4] | **已同步**：`tc_eval_const_rhs` 含 `TC_RHS_SELF_MEMBER`（与 `static let M = Self.K` 同口径）；AOT `tc_aot_emit_const_struct_expr`／`_assign` 接上 `CONST_REF`／`SELF_MEMBER`／`operand_assign` 的常量 struct／memblock 分支；语料 `tests/valid/self_const_agg.tc`、`const_struct_copy.tc`、`const_memblock_copy.tc` |
| `.count`（`memblock_count_access`）作 RHS：基址为 `memblock<T, N>` **绑定**（局部/顶层 `var`/`let`、`Self.<名>`、`<模块>.<名>`、形参），值为 `N`；常量基址在编译期折叠 | §3.2、§6.7、[语言标准 §3.8.5、附录 A] | **已同步**：AOT 常量绑定按 `binding.type` 折叠声明 `N`；语料 `tests/valid/memblock_count_rhs.tc`（四类基址 + operand／常量上下文对照） |
| `<模块>.<名> = rhs` 作**整绑定赋值目标**（附录 A `assignment` 含 `imported_member_name`）：解析为可写 `static var` → 接受；`static let` → `TC_CE_CONSTANT_ASSIGNMENT`；跨模块 `private` → `TC_CE_PRIVATE_MEMBER_ACCESS` | §6.2、§11.1、§3.2、[语言标准 附录 A、§4.4、§11] | **已同步**：Qual 已出现于本文件 `import` 时解析器直接产出 `ASSIGN`；其余单点 `FIELD_ASSIGN` 由 `tc_reclassify_qualified_binding_assign` 兜底重分类，复用整绑定赋值规则；语料 `tests/modules/import_member_assign_{ok,dep}.tc` 与负例 `member_assign_{readonly,private,type_mismatch,bad_qual}.tc` |
...
对于前导零判定，十进制字面量以 `0` 开头且后接数字字符（包括通过下划线 `_` 分隔符连接的数字）时，视为非法前导零（[语言标准 §2.3.4]）；该形态不符合附录 A 的 `dec_literal` 产生式，属**语法拒绝**（第 3 阶段 `TC_CE_SYNTAX`），不得改用 `TC_CE_LITERAL_OUT_OF_RANGE` 或 `TC_CE_LITERAL_TYPE`。忽略分隔符后仍按数字序列判定。
...
本节是实现查阅用的**索引表**。宽度语义与公式的权威定义在 [语言标准 §3.8.2]、[语言标准 §3.9.3]、[语言标准 §3.10.5]；本表不得另行引入与语言标准冲突的常数或例外。`tc_sizeof_bits` / AOT 布局 / `ptr_size` 内联必须与下表及所指条款一致。
...
| `isize` / `usize` | 目标平台指针宽度（32 或 64） | [语言标准 §3.2.1]、[语言标准 §3.8.2] |
| `ptr<T>`（任意合法所指 `T`） | `sizeof_bits(usize)` | [语言标准 §3.10.5] |
...
- `void` 无 `sizeof_bits`；不得作为值槽、`memblock` 元素或（除结构体字段指针自引用例外外的）`ptr` 所指。
- 嵌套 `memblock` / 嵌套结构体按上表递归展开；布局与宿主 ABI 对齐无关。
- `ptr_size(T, …)` 的编译期内联结果即为上表对所指类型 `T` 的取值（[语言标准 §6.8.8]）。
- 结构体字段中的 `ptr<S>`（`S` 为正在定义的本结构体）宽度仍为 `sizeof_bits(usize)`，不依赖 `S` 最终布局（[语言标准 §3.9.1]）。
...
设计动机：`N` 是**绑定级容量约束**（声明时确定的元素个数规划，并与运行时长度头部一致），不是元素种类的一部分。将其留在类型等价之外，可使「同元素类型」的身份判定保持简单（`tc_type_equals` 只看 `T`），而在赋值/传参等需要整块兼容的上下文再用独立的 `N` 比较报 `TC_CE_MEMBLOCK_SIZE_MISMATCH`。实现不得把这两层合并成「不同 `N` 即不同类型」。

编译器必须：

- **记录规划个数 `N`**：绑定的完整类型（`const TcType*`，`params.memblock_type.count`）持有声明 `N` 的数学值及元素类型 `T`。`N` 来源限于 [语言标准 §3.8.1] 的编译期 `usize` 常量：整数字面量（**含 `u`/`U` 后缀**——按 [语言标准 §2.3.1] 在无符号上下文合法；负号仅可与无后缀字面量组合）或类型为 `usize` 的 `let` / `static let` 标识符（含 `Self.` / 导入限定名）。来源不合法（运行时 `var`、形参、非常量表达式）**或数学值 `< 1`** → `TC_CE_CONSTANT_EXPRESSION`。
- **计算并记录类型级宽度**：`sizeof_bits(memblock<T, N>) = sizeof_bits(usize) + N × sizeof_bits(T)`（速查见 §3.0.1），其中 `sizeof_bits(T)` 按 [语言标准 §3.8.2] 的宽度表计算，`sizeof_bits(usize)` 为目标平台指针宽度。该宽度在编译期完全确定，供 memblock 整体赋值和按值传参使用。
...
- **类型定义唯一性**：结构体表按 `(定义模块名, 结构体名)` 定界；不同模块可同名。同一模块内结构体名不得与标量类型名、`memblock`/`ptr` 类型名、关键字冲突（使用保留字属**语法拒绝** `TC_CE_SYNTAX`）。三类名称冲突分别定码（[语言标准 §3.9.1]）：同一模块重复定义同名结构体 → `TC_CE_DUPLICATE_STRUCT`；与同模块函数名或值绑定名冲突 → `TC_CE_FUNCTION_NAME_CONFLICT`；与导入名冲突 → `TC_CE_IMPORT_NAME_CONFLICT`。
- **结构体名解析**：类型语法为 `identifier | imported_member_name`（[语言标准附录 A]）。Parser 将未决名（裸名或 `Mod.Name`）写入 `TcType.pending_name`；Analyzer 在注册结构体表时按**当前文件**的 `import` 列表解析。
  - 裸标识符仅解析为**当前模块**已定义的结构体；不得因依赖库中存在同名 `public struct` 而接受裸名（导入方写 `Player` → `TC_CE_UNDEFINED_STRUCT`）。
  - `<模块名>.<结构体名>` 要求限定符为本文件已成功 `import` 的库（传递依赖不计入）；未 import 或目标库无该结构体 → `TC_CE_UNDEFINED_STRUCT`；存在但为 `private` → `TC_CE_PRIVATE_MEMBER_ACCESS`（不得改报 `TC_CE_UNDEFINED_STRUCT`）。
  - 值构造器名解析成功后规范化为定义模块的 `<模块名>.<结构体名>`，供后续查找使用。
- **字段类型检查**：字段类型仅可为标量（`scalar_type`）、`memblock<T, N>`（带 `N` 规划）、`ptr<T>`，或结构体类型名。结构体名按 [语言标准 §3.9.1] 的位置规则分流：值位置（字段类型本身或 `memblock` 元素类型）允许本模块源序更早已成功定义完毕的结构体，以及经导入限定解析到的、目标库中已定义完毕的 `public struct`；禁止当前结构体自身与本模块前向引用。指针所指位置允许完整类型或**正在定义的本结构体**（指针自引用），仍禁止其它未完成定义的结构体名。值位置自引用 → `TC_CE_STRUCT_VALUE_SELF_REF`；前向引用或未定义结构体 → `TC_CE_UNDEFINED_STRUCT`。`void` 字段类型不合法。
...
- **本实现固定 64-bit-only 目标字长**：`isize`/`usize`/指针宽度与 memblock 长度头均为 64 位。以编译期断言约束宿主为 64 位（`sizeof(void *) == 8`）。32 位目标另行立项。
...
| 转换形式 | 静态验证 | 运行时检查 | 说明 |
| -------- | -------- | ------------ | ---- |
| `cast(T, operand)` | 源、目标可为整数/浮点/`bool`/`ptr<U>`→`ptr<T>`。**指针类型转换不附加等宽条件**：所有指针值恒等宽（§3.2），仅要求 `U`、`T` 均为 `void` 以外的完整类型，不满足该要求 → `TC_CE_TYPE_MISMATCH`；源为 `nullptr` 时无所指 `U`，由目标 `ptr<T>` 定型；`ptr<T>` 不可 `cast` 到整数/浮点 | 严格模式：范围检查触发 `TC_RE_CAST_OVERFLOW`；浮点到整数 NaN/无穷 → `TC_RE_CAST_OVERFLOW` | 数值转换含范围检查与规定舍入 |
| `cast(T, truncate, operand)` | 源与目标必须均为整数且目标更窄（`m < n`）；`m ≥ n` → `TC_CE_MODE_MISMATCH`；不接受非整数类型 | 无溢出检查；保留低 `m` 位 | 仅整数缩窄 |
| `bitcast(T, operand)` | 源与目标必须等宽 → `TC_CE_BITCAST_WIDTH`；`bool` 不可参与 → `TC_CE_TYPE_MISMATCH`；memblock/结构体不可参与 → `TC_CE_TYPE_MISMATCH`；允许 `ptr`↔等宽整数及 `ptr`↔`ptr`；**禁止 `ptr` ↔ 浮点**（→ `TC_CE_TYPE_MISMATCH`，[语言标准 §6.6.6]、[语言标准 §3.10.9]） | 无运行时检查；位模式不变 | 等宽位重解释 |

**实现要点**：`cast`/`bitcast` 目标 `T` 在 Parser 阶段经完整类型语法解析（`tc_parse_type_syntax`）；Pass2 将目标 intern 为稳定 `const TcType*`（写入 RHS 的 `target_type`），并以期望类型做 `tc_type_equals` 校验；**指针 `cast` 不得再以「所指类型等宽」为条件**（指针值恒等宽，[语言标准 §3.7]）；`bitcast` 仍须校验源目标位宽相等并排除 `ptr` ↔ 浮点。Executor/AOT **只读**使用已解析指针，不得再调用 `tc_type_intern` 写入 `type_table`。不得因内部仍用 `TcTypeTag` 投影而拒绝合法 `ptr` 目标。
...
`TC_CE_PRIVATE_MEMBER_ACCESS` 属于名称/可见性解析：函数与 `static` 成员在第 6/7 阶段；导入结构体类型名与值构造器在 4c 与 4d 之间的结构体表注册中。定位到限定名中的成员名标识符，关联位置为该私有成员的声明名；不得改报 `TC_CE_UNDEFINED_VARIABLE`、`TC_CE_UNDEFINED_FUNCTION` 或 `TC_CE_UNDEFINED_STRUCT`（[语言标准 §4.4]）。公开 `static var` 的跨模块读写（含作为 `read` 目标）按 [语言标准 §4.4] / [语言标准 §10.3] 保留完整语义，不得降级为警告。
...
- 嵌套调用（如 `mul` 内嵌 `add`） → **语法拒绝** `TC_CE_SYNTAX`（调用形式不属于附录 A 的 `const_operand`，与指针指令、`memblock_load` 等非 `const_rhs` 形态同等对待，[语言标准 §5.2.1]、[语言标准 §1.3]）；
...
- 逻辑短路读边与静态布尔三态在第 10 阶段判定（§1.2），并与本节 §9.2.1 的 CFG 边裁剪衔接（路径语义见 [语言标准 §9.2]）；
...
TC 浮点 `mod` 的取模核心是「商向零截断」，**以 [语言标准 §6.3.7] 为准**；若实现选择委托宿主余数函数（如 C99 `fmod`），必须逐项验证其语义与本节一致（含符号、零结果符号、`a` 有限而 `b` 为无穷时按位等于 `a`、以及非规格化数不被冲刷），并**不得依赖宿主浮点环境**（舍入模式、异常标志、FTZ/DAZ，[语言标准 §6.3.2]）。不得因为优化而采用向负无穷、向最近或向偶数截断。对 strict 模式，必须先判断 NaN/无穷/零除数的特殊情形，再进入实际求值。
...
**`const_rhs` 额外约束**：`const_rhs` 仅接受 [语言标准 §5.2.1] 的原子表达式与单层调用；`ptr_load`、`ptr_address`、`ptr_add`、`ptr_sub`、指针比较、`memcopy_unsafe`、`memblock_load`、`memblock_copy`、嵌套调用、`ptr_size` 的嵌套均不在其产生式内。**违反 → 语法拒绝 `TC_CE_SYNTAX`**（[语言标准 §1.3]、[语言标准 §5.2.1]，不得改报 `TC_CE_CONSTANT_EXPRESSION`）。
...
| `ptr_address(T, ident)` | `T`（所指类型，非 `void`） | `ident`: `var` / `static var` / 形参 | `let` / `static let` → `TC_CE_CONSTANT_ASSIGNMENT`；`T` 须与 `ident` 声明类型严格一致。取址本身合法；经该指针再 `ptr_store` / `memcopy_unsafe` 时，所指为形参 → `TC_CE_CONSTANT_ASSIGNMENT` | 否 |
...
| `memcopy_unsafe(T, dst, d_idx, src, s_idx, len)` | `T`（元素类型，非 `void`） | `dst`, `src`: `ptr<T>`；`d_idx`, `s_idx`, `len`: 整数 | `nullptr` → `TC_RE_NULL_POINTER_DEREFERENCE`；`len` / `d_idx` / `s_idx` 编译期可确定为负 → `TC_CE_MEMCOPY_UNSAFE_INVALID_RANGE`；编译期不可确定的负值在运行时 → `TC_RE_MEMCOPY_UNSAFE_INVALID_RANGE`；不检查越界；执行语义为 memmove | 否（语句） |
...
5. 按本节 §9.2.1 从函数唯一入口遍历 CFG（路径语义见 [语言标准 §9.2]）；任何不在可达集合中的语句节点报 `TC_CE_UNREACHABLE_STATEMENT`。只有入口可达节点参与确定初始化固定点和函数末尾检查；不可达代码的词法、语法、名称和类型检查仍执行。
...
标签查找必须按 [语言标准 §7.3.2] 从 `goto` 所在块沿祖先链由内向外进行，最近同名标签胜出。若可见链查找失败且当前函数存在多个不可见同名候选，诊断类别固定按"后代块 `TC_CE_JUMP_INTO_BLOCK` 优先于不可比块 `TC_CE_JUMP_INCOMPATIBLE_BLOCK`"选择；同类别内以源位置最早的标签作为关联位置。不得使用标签表或哈希表的自然遍历顺序选择目标或诊断。
...
顶层和全部函数 CFG 构建完成后，CFG 类错误按源位置统一选择，不依赖内部分析顺序。同一位置依次为：`break` / `continue` 位置 → 按本节 §9.2.1 从控制流域入口不可达的语句 → 确定初始化 → 缺少返回。存在多个不可达语句时，选择源位置最早者，不依赖 CFG 节点遍历顺序。所有 `goto` / `label` 合法性诊断已在第 6 阶段（子阶段 6a–6c）按 §7.5 完成；非法跳转不进入 CFG，也不参与本阶段的错误选择。
...
- 形参只读绑定：入口已初始化。对形参**绑定本身**赋值、字段赋值或作为 `read` 目标 → `TC_CE_PARAMETER_ASSIGNMENT`。对形参做 `ptr_address` 本身合法；经该指针再 `ptr_store` / `memcopy_unsafe` → `TC_CE_CONSTANT_ASSIGNMENT`（[语言标准 §6.8.3]、[语言标准 §8.1.2]），不得改报 `PARAMETER_ASSIGNMENT`。
- `void` 不得用作形参类型（语法拒绝）；`ptr<void>` 不合法（静态语义拒绝）。
- `memblock<T, N>` 形参在传参时须比较两侧声明的 `N` 数学值，不相等 → `TC_CE_MEMBLOCK_SIZE_MISMATCH`（见 §3.1）。
- `ptr<T>` 形参按值传递：复制指针值（地址），调用者与函数体共享所指对象。函数体内对形参做 `ptr_load` 合法。对该指针值做 `ptr_store` 写入的是所指对象，合法性按所指外层绑定判定，不因指针本身是形参而改报 `PARAMETER_ASSIGNMENT`。多参数均为 `ptr<T>` 值时可能互为别名。
...
| 参数 | 函数入口时已初始化，只读；不得对绑定本身赋值或作为 `read` 目标（`TC_CE_PARAMETER_ASSIGNMENT`）。经 `ptr_address` 后再 `ptr_store` / `memcopy_unsafe` → `TC_CE_CONSTANT_ASSIGNMENT` |
...
`TC_CE_VAR_MISSING_INIT` 不得降级为通用语法错误；`TC_CE_UNINITIALIZED_VARIABLE` 不得替代名称解析错误。0.0.44 不定义 `TC_CE_GOTO_SKIPS_VAR_INIT`，因为未初始化是跨全部控制流种类的通用性质，不应绑定到单一语句。
...
0.0.44 不定义 `TC_CE_GOTO_SKIPS_VAR_INIT`、常量循环依赖专用错误、跨块引用专用错误或重复的 overflow-mode 错误：它们分别由确定初始化、源序可见性/名称解析，以及"EBNF 先行、模式语义随后"的统一规则处理。语言正文对 `funcall` 实参缺失/重复/乱序等已给出专用码（见语言标准附录 B.12）；本说明书 §8.2 / §11.4.2 必须采用这些码，不得合并为笼统 `TC_CE_SYNTAX` 或 `TC_CE_TYPE_MISMATCH`（除非附录 A 已在语法阶段拒绝）。
...
下列各表是 0.0.44 完整、权威的标准错误码对照。语言错误码使用 `TC_CE_*`（编译期/静态）与 `TC_RE_*`（运行时）前缀，另含实现资源失败码 `TC_ERR_OUT_OF_MEMORY`。语言错误码、打印名与诊断类别一一对应，不得合并、别名化或以其他错误码代替；布尔 `xor` 与浮点 `mod` 复用既有错误码。本表镜像语言标准附录 B（86 码），另记 `TC_ERR_OUT_OF_MEMORY`。
...
| `TC_CE_UNREACHABLE_STATEMENT` | `UnreachableStatement` | 不可达语句 | CFG | 完成静态条件边裁剪后，该语句节点不能从当前顶层或函数控制流域的唯一入口到达（本节 §9.2.1；[语言标准 §9.2]） |
| `TC_CE_PARAMETER_ASSIGNMENT` | `ParameterAssignmentError` | 参数赋值错误 | 名称/类型 | 对形参**绑定本身**赋值（含整绑定和字段赋值），或将其作为 `read` 目标。不含经 `ptr_address` 取址后的 `ptr_store` / `memcopy_unsafe`（后者用 `TC_CE_CONSTANT_ASSIGNMENT`；[语言标准 §3.10.9]、[语言标准 §8.1.2]） |
...
下表固定 [语言标准 §3.10] / [语言标准 §6.8] 所需的指针与 `memcopy_unsafe` **专用**诊断 2 码（空指针运行时码亦列于 §11.4.1，本表交叉列出以便检索，不另计）。跨类型指针比较复用 `TC_CE_TYPE_MISMATCH`，不另设专用码。

| 错误码 | 打印名 | 诊断类别 | 阶段 | 触发条件 |
| --- | --- | --- | --- | --- |
| `TC_CE_MEMCOPY_UNSAFE_INVALID_RANGE` | `MemcopyUnsafeInvalidRange` | memcopy 区间非法（静态） | 静态 | `memcopy_unsafe` 的 `length` / `dst_idx` / `src_idx` 在编译期可确定数学值 `< 0`（字面量或 `let` / `static let` 常量来源；[语言标准 §6.8.9]、附录 B.11） |
