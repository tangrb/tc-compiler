...
| 经 `Self.` 解析到的 `static let` 可作 `let` 初始化器的常量来源（标量 / struct / memblock 同口径）；常量聚合值拷贝须深拷贝（生成代码不得嵌入分析期堆指针） | §3.2、§5.2.1、§6.7、[语言标准 §5.2.1、§3.9.4、§3.8.4] | **已同步**（观察-④）：`tc_eval_const_rhs` 补 `TC_RHS_SELF_MEMBER` 分支（此前落到「invalid constant expression」，而 `static let M = Self.K` 走专用分支故两个常量上下文不一致）；AOT 新增 `tc_aot_emit_const_struct_expr`／`_assign` 并接上 `CONST_REF`／`SELF_MEMBER`／`operand_assign` 的常量 struct／memblock 分支；语料 `tests/valid/self_const_agg.tc`（`let x: int32 = Self.K`、`let`／`var p: Pair = Self.spc`、`return Self.spc`、const memblock 拷贝）、`const_struct_copy.tc`、`const_memblock_copy.tc` |
| `.count`（`memblock_count_access`）作 RHS：基址为 `memblock<T, N>` **绑定**（局部/顶层 `var`/`let`、`Self.<名>`、`<模块>.<名>`、形参），值为 `N`；常量基址在编译期折叠 | §3.2、§6.7、[语言标准 §3.8.5、附录 A] | **已同步**（观察-⑤）：AOT 常量绑定路径改按 `binding.type` 折叠声明 `N`（此前仅按名查符号，限定名查不到 → `code generation failed`）；语料 `tests/valid/memblock_count_rhs.tc`（`run_diff_test` ＋ `run_check_ok`，四类基址 + operand／常量上下文对照） |
| `<模块>.<名> = rhs` 作**整绑定赋值目标**（附录 A `assignment` 含 `imported_member_name`）：解析为可写 `static var` → 接受；`static let` → `TC_CE_CONSTANT_ASSIGNMENT`；跨模块 `private` → `TC_CE_PRIVATE_MEMBER_ACCESS` | §6.2、§11.1、§3.2、[语言标准 附录 A、§4.4、§11] | **已同步**（观察-⑥）：分析器把「单点限定目标」由字段赋值**重分类**为整绑定赋值（`tc_reclassify_qualified_binding_assign`），复用既有赋值规则；语料 `tests/modules/import_member_assign_{ok,dep}.tc` 与负例 `member_assign_{readonly,private,type_mismatch,bad_qual}.tc` |
