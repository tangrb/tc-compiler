...
> **不代表实现侧零未决**：本文只登记 libtc 调用契约层面的口径。审计报告 §5 复核的 9 项既有未关闭差异（导入限定名成员引用、`Self.<名>`/限定名取址、重复实参诊断码、`#lib` `static var` 缺初始化器、`memcopy_unsafe` 常量负区间静态检查等）**已全部闭合**，本轮另闭合 4 项观察项（限定名成员可见性、`Self.` 常量来源与常量聚合值深拷贝、`.count` 常量绑定路径、`<模块>.<名> = rhs` 赋值目标）；逐条状态与提交以《TC 0.0.44 符合性整改进度台账》为准（本文不回填）。两个内存入口（`tc_compile_source_opts` / `tc_compile_source`）均执行 4b–4d 导入解析（见 §15.3）。
...
> 上表是**能力范围**，不代表实现侧零未决；当前**无开放的符合性差异**（审计 §5 九项与本轮观察项均已闭合，逐条状态以《TC 0.0.44 符合性整改进度台账》为准，本文不回填），本页只描述 API 契约与阶段覆盖。
