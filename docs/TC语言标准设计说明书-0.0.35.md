...
- [2. 词法结构](#2.词法结构)
- [3. 类型系统](#3.类型系统)
- [4. 程序结构与模块系统](#4.程序结构与模块系统)
- [5. 声明](#5.声明)
- [6. 表达式与运算](#6.表达式与运算)
- [7. 控制流](#7.控制流)
- [8. 函数与调用模型](#8.函数与调用模型)
- [9. 作用域、生命周期与确定初始化](#9.作用域、生命周期与确定初始化)
- [10. 标准 I/O](#10.标准I/O)
- [11. 诊断与错误处理](#11.诊断与错误处理)
- [12. 资料性示例](#12.资料性示例)
- [附录 A：形式化语法（EBNF）](#附录A形式化语法EBNF)
- [附录 B：与 C99 语言标量语义对照](#附录B与C99语言标量语义对照)
- [附录 C：各类型边界常量速查（C99 对照）](#附录C各类型边界常量速查C99对照)
...
| 原则 | 设计意图 | 详见 |
| ---- | -------- | ---- |
| 逐条执行 | 每条语句至多一个运算，无表达式嵌套，学习者可逐行暂停检查状态 | §6.1.1 |
| 显式类型标注 | 声明必须标注具体类型，类型信息始终可见 | §5.1 |
| 类型参数首位 | 运算类型作为第一个参数显式给出，无需猜测结果位宽或符号性 | §6.3 |
| 缩进强制 | 块结构由缩进界定、`end` 结束，控制流范围一目了然 | §7.1.2 |
...
| 原则 | 设计意图 | 详见 |
| ---- | -------- | ---- |
| 声明时必须初始化 | `var` 统一带 `= <初始化器>`，杜绝声明后忘记初始化的错误模式 | §5.1 |
| 声明与再赋值分离 | 首次用 `var … = …`，其后省略 `var`；不存在模糊的首次写入路径 | §5.1, §6.2 |
| 运算函数化 | 所有运算以 `op(类型, 操作数…)` 调用，无优先级、结合性或隐式重载 | §6.3–6.6 |
| 同类型运算 | 操作数必须同类型，无混合精度算术 | §3.5, §6.3.1 |
| 单调用语句 | funcall 实参与 return 操作数限于 `operand`，禁止嵌套 | §8.2, §8.3 |
| 字面量类型显式 | 后缀唯一决定字面量类型，无推断歧义 | §2.3–2.5, §3.6 |
| 函数内跳转边界 | goto/label 限于函数体内，顶层禁止使用 | §7.3 |
...
| 原则 | 设计意图 | 详见 |
| ---- | -------- | ---- |
| 默认严格模式 | 有符号整数与浮点运算默认检测异常并报错；wrap/ieee 须显式声明 | §6.3.2 |
| 初始化安全 | 语法层声明时初始化 + 控制流层完整 CFG 确定初始化分析；可达的未初始化读取均为静态错误 | §5.1, §9.2 |
...
| 调用图无环 | 禁止递归（直接与间接），栈深度编译期可确定上界 | §8.6.3 |
...
下列代价是 0.0.35 为满足 §1.1–§1.3 而**显式接受**的，不是文档疏漏或后续补丁空间：
...
本节上限是只依赖 Token 自身的第 2 阶段词法值检查：在最长匹配形成候选整数字面量后、进入语法解析前执行，失败固定报告 `TC_ERR_LITERAL_OUT_OF_RANGE`。它不同于第 6/7/8 阶段依据变量、形参或返回类型进行的上下文字面量检查；两类检查的阶段分工见 §11.1。
...
`#program` 和 `#lib` 是模式指令 Token，只能出现在 §4 规定的文件首个有效行。`call` 不是 0.0.35 的调用关键字；函数调用必须写作 `funcall`。`void` 只允许出现在函数返回类型位置。
...
TC 0.0.35 支持 **整数类型**、**浮点类型**、**布尔类型**、**内存块类型**，并以 `void` 表示函数不返回值：
...
编译器必须在分析函数体和顶层语句前收集全部函数签名，因此 `#lib` 中函数可调用定义在其后的函数；调用图限制见 §8.6。每个函数构成独立控制流域，只有函数控制流域可以建立标签表和 `goto` 边。
...
- 编译器对每条 `var` 做语法/形态检查：缺 `=` 或初始化器 → `TC_ERR_VAR_MISSING_INIT`（打印名 `VarMissingInitializer`，§11.1 / §11.5）。此检查与控制流无关，亦不依赖数据流分析；不得仅报笼统 `TC_ERR_SYNTAX`。
- 同一作用域内不可重复定义同名变量（作用域包括顶层、函数、`if`/`while` 块，以及 `then`/`else` 各自的互斥子块，见 §9.1、§7.1.5、§7.2.3）。跨类型值须先 `cast`；函数返回值则必须与接收类型严格相同。
- 声明带初始化器不等于该语句在所有路径上均已执行；完整 CFG 的确定初始化规则见 §9.2 与 §7.3.4。
...
- 为统一源级可达性诊断，每条 `let` 仍作为一个无运行时效果的 CFG 身份节点参与 §9.2.1 的可达性分析；该节点的数据流传递函数为恒等，不生成运行时指令
- 常量 不可 作为赋值语句的左值
...
三态判定与短路读边是两个独立分析结果。只要条件调用的任一操作数含 `var`，整个条件仍为 `unknown`，即使其左操作数足以由 `false and ...` 或 `true or ...` 决定运行时结果；§6.5.2.2 仍可单独删除右操作数的读取边。实现不得把该读边裁剪升级为本节未授权的 CFG 真/假边裁剪，否则会改变 `TC_ERR_UNREACHABLE_STATEMENT` 的合法程序集合。
...
0.0.35 先以附录 A EBNF 判定表达式形态是否合法（**语法拒绝**，§1.4）；仅对 EBNF 已接受的表达式，再以“操作 × 类型 × 模式”矩阵判定是否构成**静态语义拒绝**：
...
0.0.35 不支持浮点 `wrap`，也不接受冗余的无符号 `wrap`；具体错误类别必须先按上述 EBNF 边界确定。
...
strict 指令必须先完成上述分类，再以无限精度数学结果和目标格式正确舍入判定上溢/下溢；不得直接依赖 C99 表达式是否设置宿主 `fenv` 标志。发生运行时错误时，该指令没有结果提交，规则见 §11.3。
...
| R7       | 合法的函数内 `label` 不产生新的缩进层级、不开启代码块。`label` 行缩进与同作用域其它普通语句一致；其后仍属同一作用域、且非新子块内的语句，缩进必须与该 `label` 行完全一致。仅当后续语句本身开启 `if`/`while` 子块时，子块内部才按 R3–R6 增加缩进（详见 §7.3.2）。 |
...
`if` 可出现在顶层或函数体内。函数体各分支可使用 `return`；非 `void` 函数是否覆盖全部正常退出路径按 §8.3.2 和 §8.4 的函数 CFG 判断。顶层 `return` 是静态错误。函数定义本身不得出现在任何 `if` / `else` 分支内。
...
每次条件为 `true` 都进入一个新的循环体块实例；规范上的局部生命周期逐轮结束并重建。实现可以复用固定物理槽，但必须在确定初始化状态和可观察值上等价于重新进入。`continue`、正常回边和条件失败均按 §9.2.1 的边规范化处理。
...
2. **变量生命周期**：若跳转跳出某作用域（如 `then` 块），该块内的局部变量按 §9.1 / §9.2 **立即销毁**（与 `end` / `break` 离开块等价）；后续任何对该块内变量的引用，静态分析会报"未定义标识符"。
3. CFG 边：每条合法 `goto` 都必须形成从跳转语句到目标 `label` 节点的显式 CFG 边；`label` 再通过普通落入边连接其续点。向前、向后、同层与向外跳转使用同一表示。
4. 确定初始化：编译器按 §9.2 在包含该边的完整 CFG 上求固定点。任何变量读取、赋值目标或 `read` 目标若不在当前 `IN` 集合中，触发静态错误 `TC_ERR_UNINITIALIZED_VARIABLE`。
...
- 形参与任何局部值绑定若与全局函数同名，统一报告 `TC_ERR_FUNCTION_NAME_CONFLICT`；顶层嵌套块的值绑定也按同一规则处理，不因 `funcall` 与普通 RHS 使用不同查找入口而获准同名。函数内任何层级都不得声明与形参同名的 `var` / `let`；这一参数保护冲突继续报告 `TC_ERR_DUPLICATE_DEFINITION`。两类冲突的定位与阶段见 §11.1.1 / §11.5。
- 编译器在分析任何函数体和顶层语句之前收集全部函数签名。函数可调用定义在其后的函数，但调用图必须无环（§8.6.3）。
...
0.0.35 中 `#program` 不得定义函数，`#lib` 也没有非 `static` 的顶层值作用域，因此不再存在“同文件顶层 `var` / `let` 对函数可见性越界”的诊断场景。为使忘记 `Self.` 的诊断唯一且可观察，编译器必须为当前 `#lib` 模块建立**本库顶层成员名索引**（全部 `func` / `static let` / `static var` 的声明名，不分 `public` / `private`）。该索引只用于错误分类，绝不把这些名字注入函数体的普通裸名查找。函数内普通裸名查找失败后：
...
顶层与每个函数是彼此封闭的控制流域；只有函数建立标签表。`return` 只退出当前函数，`break`/`continue` 只解析到当前函数内最内层 `while`。`goto`/`label` 的查找、块关系和诊断严格按 §7.3 执行并在函数边界停止：跨函数或跳向顶层的边不进入 CFG，不同函数中的同名标签互不相关。
...
- **标签作用域**：标签只存在于函数内，遵循与函数局部变量（`var`）和常量（`let`）相同的块级可见性，并额外受函数边界与范式隔离约束：
  - `label` 必须具有 `func` 词法祖先，且 `while` 代码块内禁止定义 `label`（§7.2.4）。合法标签只能出现在函数体代码块或函数内 `if` 的 `then` / `else` 代码块中。`label` 不开启新代码块或作用域，但必须形成新的 CFG 基本块入口。
...
- **源序可见性**：`var` / `let` 绑定从其声明结束后的词法位置开始可见；定义自身的初始化器或 RHS 不可引用正在定义的绑定，亦不可引用尚未声明的值标识符。`var` 在某条运行时路径上是否已执行初始化由 §9.2 的确定初始化分析判定；`let` 在第 9 阶段统一求值并内联，不以运行时路径是否经过其声明行为条件，因此 `goto` 绕过 `let` 不影响后续词法可见位置对该常量的使用。函数签名是另一例外：全部函数签名在函数体分析前收集，因此函数可以调用定义在其后的函数，但调用图不得形成环。
- **变量生命周期**：
  ...
  - 局部变量（块内）：进入作用域时为该作用域的全部 `var` 绑定建立固定槽位；执行到对应 `var` 时计算 RHS、写入槽位并成为已初始化。反向跳转或下一次循环迭代重新执行同一 `var` 时，重新初始化同一词法槽，不重复分配。离开当前块实例时统一结束其局部生命周期——离开方式包括对应 `end` 的正常落入、循环体正常回边、`break`、`continue`、`return` 或向外 `goto`；实现可以复用物理槽，但必须清除该块实例的确定初始化状态。块内 `let` 随词法作用域结束而不再可见（本身无运行时槽，见 §9.2）。
...
1. 在 §5.2 常量求值和静态布尔三态判定完成后构建 CFG；静态 `true` / `false` 条件删除不可能边，`unknown` 条件保留真、假两边。
...
- 任何可达的运行时 `var` / 参数读取、赋值目标或 `read` 目标都要求对应运行时绑定属于当前 `IN`。`let` 只要求名称可见且已成功求值，不属于 `U` / `IN`；逻辑短路的右操作数是否可达按 §6.5.2.2 判定。
...
`read` 先完整扫描并验证一个输入 Token，再转换并提交目标值；只有全部步骤成功后才覆盖目标变量。失败时目标槽保持原值，随后程序按 §11.3 终止。实现不得受当前 locale、宿主 `strto*` 接受的扩展拼写或宿主浮点中间精度影响。
...
已匹配附录 A `format_specifier` 形态后，重复标志、宽度或精度超出上述范围，以及转换符不适用的标志/精度，均在 §11.1 的第 6 阶段报告 `TC_ERR_FORMAT_SPECIFIER`。不符合该 EBNF 的形式在第 3 阶段按语法规则报告 `TC_ERR_SYNTAX`。`%p`、C99 空格标志、长度修饰符（如 `hh`/`l`/`ll`/`L`）和 `*` 均不受支持；TC 的类型已由第一个参数显式给出。
...
除 §11.1.2～§11.1.7 或其他规范性条款已经给出更专用的定位规则外，普通静态语义诊断的**主位置**按下列规则固定：
...
函数参数名在整个函数内是受保护绑定。函数签名检查已成功后，任意层级的 `var` / `let` 声明若与当前函数形参同名，在第 6 阶段报告 `TC_ERR_DUPLICATE_DEFINITION`：主位置为该内层声明的标识符，关联位置为对应形参的声明标识符；存在多个此类冲突时按主位置源序选择最早者。该规则是参数保护的专用扩展，不禁止内层块按 §9.1 屏蔽普通外层局部绑定。
...
| `shl` / `shr` | 2 | 模式位置的合法性仍由 EBNF 与 §6.3.1 判定 |
...
缺少右括号、顶层逗号结构破损或无法识别内建调用外壳时，使用 `TC_ERR_SYNTAX` 而非 `TC_ERR_OPERAND_COUNT`。嵌套调用也不属于操作数数量错误：`let` RHS 中的嵌套调用按本节后文报 `TC_ERR_CONSTANT_EXPRESSION`，其他不允许嵌套的位置报 `TC_ERR_SYNTAX`。`funcall` 的项是命名实参，不纳入本错误码；其缺失、重复、未知与顺序错误分别使用 §11.1.3 的专用诊断。`return`、`if` / `while`、变量/常量定义与其他语句形态也不属于内建固定元数调用外壳，沿用各自的语法或专用诊断。
...
第 5 阶段的候选错误包括函数重名、签名内名称冲突，以及全局函数名与顶层值作用域直接值绑定的冲突。多个候选先按本节规定的主位置选择源序最早者；同一主位置再按以下顺序：函数重复定义 → 函数名与顶层值作用域直接值绑定冲突 → 参数重名 → 参数名与全局函数名冲突。顶层嵌套块和函数体 `var` / `let` 的同名冲突在第 6 阶段按 §11.1.1 处理。
...
标签查找必须按 §7.3.3 从 `goto` 所在块沿祖先链由内向外进行，最近同名标签胜出。若可见链查找失败且当前函数存在多个不可见同名候选，诊断类别固定按“后代块 `TC_ERR_JUMP_INTO_BLOCK` 优先于不可比块 `TC_ERR_JUMP_INCOMPATIBLE_BLOCK`”选择；同类别内以源位置最早的标签作为关联位置。不得使用标签表或哈希表的自然遍历顺序选择目标或诊断。

编译器必须先收集全部函数中通过上述上下文检查的标签，再解析任何函数内 `goto`；跨函数诊断不得依赖函数定义顺序、标签表遍历顺序或当前正在分析的函数。

本小节中的全部 `goto` / `label` 上下文、名称、控制流域和块关系错误均属于 §11.1 第 6 阶段（子阶段 6a 控制流上下文 + 6b 名称作用域预建 + 6c goto/label 名称解析）。只有完成标签绑定并通过全部上述检查的 `goto` 才能在第 11 阶段进入当前函数 CFG；CFG 阶段不再产生 `goto` / `label` 合法性诊断。

#### 11.1.6 CFG 错误优先级

顶层和全部函数 CFG 构建完成后，CFG 类错误按源位置统一选择，不依赖内部分析顺序。同一位置依次为：`break` / `continue` 位置 → 按 §9.2.1 从控制流域入口不可达的语句 → 确定初始化 → 缺少返回。存在多个不可达语句时，选择源位置最早者，不依赖 CFG 节点遍历顺序。所有 `goto` / `label` 合法性诊断已在第 6 阶段（子阶段 6a–6c）按 §11.1.5 完成；非法跳转不进入 CFG，也不参与本阶段的错误选择。
...
| ---- | -------- | ------ | -------- | -------- |
| **语法层** | `var` 声明缺少 `= <rhs>` | `TC_ERR_VAR_MISSING_INIT` | 纯形态；与控制流无关 | §5.1 |
| **名称层** | 标识符从未声明、尚未源序可见或已超出词法作用域 | `TC_ERR_UNDEFINED_VARIABLE` | 名称解析 | §9.1 |
| **数据流层** | 声明存在，但在可达使用点不满足确定初始化 | `TC_ERR_UNINITIALIZED_VARIABLE` | 完整 CFG 固定点分析 | §9.2、§7.3.4 |
...
下列各表是 0.0.35 完整、权威的标准错误码对照，共 78 项：77 个语言错误码和实现资源失败码 `TC_ERR_OUT_OF_MEMORY`。语言错误码、打印名与诊断类别一一对应，不得合并、别名化或以其他错误码代替；布尔 `xor` 与浮点 `mod` 复用既有错误码。
...
| 非法常量形态 | RHS 不符合 §5.2.1 原子表达式或单层调用的形式 | `invalid constant expression form` |
...
| `TC_ERR_OPERAND_COUNT`             | `OperandCountError`          | 操作数数量错误                | 静态        | §11.1.1 权威表列出的内建固定元数调用外壳中，值操作数数量与要求不符；不适用于 `funcall` |
...
| `TC_ERR_MODE_MISMATCH`             | `ModeMismatch`               | 模式不匹配错误                | 静态        | EBNF 已接受候选模式参数，但操作、类型与 `wrap`/`ieee`/`truncate` 不符合 §6.3.1 / §6.6；无模式参数产生式的形态使用 `TC_ERR_SYNTAX` |
| `TC_ERR_BITCAST_WIDTH`         | `BitcastWidthError`      | 位重解释宽度错误          | 静态    | `bitcast` 源、目标位宽不一致（§6.6.6）                     |
| `TC_ERR_LABEL_NOT_FOUND`           | `LabelNotFound`              | 标签未找到错误                | 静态        | 所有函数均不存在 `goto` 引用的同名标签（§7.3.3）                   |
| `TC_ERR_DUPLICATE_LABEL`           | `DuplicateLabel`             | 标签重复定义错误              | 静态        | 同一作用域内定义了同名标签                                   |
| `TC_ERR_GOTO_OUTSIDE_FUNCTION` | `GotoOutsideFunction`    | 函数外 goto 错误          | 静态    | `goto` 不具有 `func` 词法祖先（§7.3）                    |
| `TC_ERR_LABEL_OUTSIDE_FUNCTION` | `LabelOutsideFunction`  | 函数外标签错误            | 静态    | `label` 不具有 `func` 词法祖先（§7.3）                   |
...
| `TC_ERR_JUMP_INCOMPATIBLE_BLOCK`     | `JumpIncompatibleBlockError`    | 跳入不可比块错误                | 静态        | 可见链无目标且无同名后代候选时，存在与源块不可比的同名标签（§7.3.3） |
...
| `TC_ERR_FUNCTION_SCOPE_ACCESS` | `FunctionScopeAccessError` | 函数越界名称错误 | 名称（第 6b/7/8 阶段） | `#lib` 函数内普通裸名查找失败，但该名存在于本库顶层成员名索引（`func` / `static let` / `static var`）；须改用 `Self.<名>`。不再用于“同文件顶层值作用域”场景（0.0.35 中 `#program` 不得定义函数，§8.4.1） |
...
- TC **声明时必须初始化**（`var … = …`；缺省 → `TC_ERR_VAR_MISSING_INIT`，§5.1）。此外，编译器必须在完整 CFG 上证明每次变量读取前都已执行其当前生命周期的初始化；否则报 `TC_ERR_UNINITIALIZED_VARIABLE`。`goto` 跳过但未读取某变量本身不构成错误。C99 读取未初始化对象可产生未定义行为
