...
> **承接注记（2026-08-30，v0.0.41 发布后）**：本轮降级项（FP-4.5 / FP-4.6）与不在本轮强制范围的 N-12 / N-13，由 [TC-0.0.42-遗留问题清零计划](./TC-0.0.42-遗留问题清零计划.md) 承接清零。本计划作为 0.0.41 执行记录冻结；发布后发现的 `test_module` diamond 用例平台差异（测试 bug，`eb4f24b` 修复）见分析报告 §14 后记，不改变本计划 DoD 结论。
