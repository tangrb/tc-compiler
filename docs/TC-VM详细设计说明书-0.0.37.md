...
>
> **当前实现基线**：TC-VM v0.0.37（`TC_VM_VERSION`）
>
> **状态**：0.0.37 架构设计，涵盖模块系统、函数、memblock、ptr、struct 与完整 13 阶段编译管线。
>
...
| 维度 | 版本 | 含义 |
| ---- | ---- | ---- |
| 语言规范 | 0.0.37 | 本文必须满足的合法程序集合、结果和诊断阶段 |
| 编译器规范 | 0.0.37 | 确定的 13 阶段编译管线、错误码与检查顺序 |
| 本文架构 | 0.0.37 设计 | 面向当前语言能力的实现设计 |
...
- 不为未纳入 0.0.37 的递归、异常、闭包等预先定义 ABI。
- 不以更强的可选静态规则缩小 0.0.37 合法程序集。
...
0.0.37 编译器标准 §1.2 定义 13 个确定性处理阶段，VM 后端遵循此流水线。各阶段按序执行，前一阶段有错时不进入下一阶段：
...
0.0.37 引入模块系统：一个 `#program` 文件通过 `import` 引用零个或多个 `#lib` 模块。编译器必须：
...
0.0.37 的 statement 集合相较 0.0.31 增加函数、模块、memblock、ptr 与 struct 相关 statement：
...
*本文的规范性语言规则均以 [TC 语言标准 0.0.37](./TC语言标准设计说明书-0.0.37.md) 与 [TC 编译器标准 0.0.37](./TC编译器标准设计说明书-0.0.37.md) 为准。*
