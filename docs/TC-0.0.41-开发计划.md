...
> **计划日期**：2026-07-23
> **目标**：构建完整的 TC 0.0.41 编译器（VM 解释器 + AOT C99 编译器 + libtc 嵌入库）
...
TC 是面向教学与 AI 自动化编程的指令式语言。本节定义 0.0.41 版本的完整语言能力范围（不是增量，是本版本作为独立产品的完整规格）。
...
│   └── runtime/                  # 共享运行时
│       ├── tc_version.h          # 版本号 v0.0.41（含 Embed）
...
> **目标**：建立 TC 0.0.41 完整类型表示、等价判定与宽度计算
...
> **状态**：**Phase 6 已完成（2026-07-24）** — CLI `-I`/无 REPL、libtc `name`/`tc_run_program`、版本 v0.0.41；struct 运行时已补齐
...
| K4-3 | test-map.md | 0.0.41 测试映射更新 |
| K4-4 | REPL 移除 | 删除 tc_repl.c/h; 移除 --repl 选项 |
| K4-5 | 版本号 | src/vm/runtime/tc_version.h → v0.0.41; src/aot/main.c → TC_AOT_VERSION |
...
*本计划基于 TC 0.0.41 全套设计文档（语言标准、编译器标准、VM 设计、AOT 设计、libtc 设计、libtc API、VM 命令参考、合规审查报告）。所有规范要求以原设计文档为准。*
...
> **审查范围**：对照 `TC语言标准设计说明书-0.0.41.md` 和 `TC编译器标准设计说明书-0.0.41.md` 两份文档的逐项检查  
