...
- **TC-Embed**：C 宿主程序调用 TC 编译产物的零拷贝嵌入式运行时（v0.0.44）。

当前核心版本：**v0.0.44**，Embed 模块版本：**v0.0.44**。语言语法与可观察语义以 [TC 语言标准设计说明书](docs/TC语言标准设计说明书-0.0.44.md) 为唯一权威来源（语言规范与全部设计文档均为 **0.0.44**；实现版本为 v0.0.44）。
...
| 嵌入互操作 | C→TC 零拷贝函数调用、共享 `slots[]` 数据平面、`ptr<T>` 句柄编码、符号查询；VM 与 AOT 双模式 API 兼容（v0.0.44） |

0.0.44 不包含字符串类型、字节码文件格式或 JIT；无 REPL；批量文件模式支持完整控制流。`goto`/`label` 仅函数内且 `while` 外。

## 语言示例

0.0.44 源文件必须以 `#program` 或 `#lib` 开头。算术、比较等运算使用显式类型的内建调用，而不是中缀运算符。
...
完整语法与语义见 [TC 语言标准设计说明书](docs/TC语言标准设计说明书-0.0.44.md)。
...
python3 scripts/sync/check_doc_layering.py # 设计文档不得引用过程性文档
```
