...
static TcValue make_int(TcTypeKind type, uint64_t bits) {
...
static void test_negative_shift_count(void) {
    ...
    TcValue cnt = make_int(TC_INT8, 0xFF);
    TcValue out;
    int rc = 0;
    ...
    tc_diagnostic_clear(&diag);
    rc = tc_exec_shift(TC_SHIFT_SHL, TC_INT8, TC_ARITH_WRAP, &val, &cnt, &out, &diag, 1);
    check(rc == 0 && out.bits == 0, "shl wrap with count -1 (k=255) -> 0");

    tc_diagnostic_clear(&diag);
    rc = tc_exec_shift(TC_SHIFT_SHR, TC_INT8, TC_ARITH_STRICT, &val, &cnt, &out, &diag, 1);
    check(rc == 0 && out.bits == 0, "shr with count -1 (k=255) -> 0");
