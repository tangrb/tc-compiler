...
static void test_format_spec_parse(void) {
    ...
    check(tc_format_spec_parse("%-+d", &out) == 1 && !out.flag_repeat,
          "parse '%%-+d' distinct flags → no repeat");
    check(tc_format_spec_parse("%0-0d", &out) == 1 && out.flag_repeat,
          "parse '%%0-0d' non-consecutive zero run → flag_repeat");
    check(tc_format_spec_parse("%0008d", &out) == 1 && !out.flag_repeat,
          "parse '%%0008d' merged zeros → no repeat");
}
