...
#
# 所有参数直接透传给 VM 测试脚本 (--asan, --ubsan, --valgrind, --leaks, --verbose, --filter)。
...
#   bash scripts/run_tests.sh --leaks         # macOS leaks 模式（需 Xcode）
#
# 构建:
...
while [ $# -gt 0 ]; do
    case "$1" in
    --asan|--ubsan|--valgrind|--leaks|--verbose)
        VM_ARGS="${VM_ARGS} $1"
        shift
        ;;
    --filter)
        ...
        ;;
    --help)
        echo "用法: $0 [--asan] [--ubsan] [--valgrind] [--leaks] [--verbose] [--filter PATTERN]"
        echo ""
    ...
    *)
        echo "未知选项: $1" >&2
        echo "用法: $0 [--asan] [--ubsan] [--valgrind] [--leaks] [--verbose] [--filter PATTERN]" >&2
        exit 1
        ;;
    esac
done
