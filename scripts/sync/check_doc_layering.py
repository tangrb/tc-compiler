...
规则（规范性）：
  **设计性质文档不得引用过程性文档。**
  ...
  - 设计性质文档（本脚本检查的对象，共 8 份）：
      语言标准、编译器标准、VM 详设、VM 命令行参考、AOT 详设、Embed 详设、libtc 设计说明书、
      语言标准设计说明书（0.0.45 草案）、编译器标准设计说明书（0.0.45 草案）
...
DESIGN_DOCS = [
    ...
    "docs/TC语言标准设计说明书-0.0.45.md",
    "docs/TC编译器标准设计说明书-0.0.45.md",
]
...
# ---- 已废弃的带版本号文档名（正文指名即违规） ----------------------------
# 现行设计文档名带 `-0.0.44` 或 0.0.45 草案后缀，属正常；此处只拦截**更旧**的档名
# （如 `libtc-api-0.0.42.md`）。
STALE_DOCNAME_RE = re.compile(
    r"[A-Za-z\u4e00-\u9fff_\-]*-(?:v?0\.0\.(?!44\b|45\b)[0-9]+)\.md"
)
