...
- **0.0.45 design documents frozen (2026-09-26)**: `docs/TC语言标准设计说明书-0.0.45.md` and `docs/TC编译器标准设计说明书-0.0.45.md` move from draft to frozen text — both header blocks now carry the freeze date, scope of effect and change policy, and the compiler standard gains a lockstep-versioning clause with the language specification; the freeze anchor is the `spec-0.0.45` tag. After the freeze only errata are accepted and any semantic change requires a version bump; this file is the single errata channel and the separate errata checklist is no longer kept. Same batch: the language spec's §3.8.2 "table above" now cites the section explicitly, and the Appendix A comment uses the production names defined in Appendix A.
