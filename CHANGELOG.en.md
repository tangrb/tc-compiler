...
- **0.0.45 design drafts (docs first, not implemented)**: added `docs/TC语言标准设计说明书-0.0.45.md` and `docs/TC编译器标准设计说明书-0.0.45.md`. Deltas vs 0.0.44: `ptr<T>` → `ref<T>` (managed reference), new `addr<T>` (linear address) and the `addr_*` family, `nullptr` split into two null literals: `none` (null reference, `ref<T>` only) and `nil` (null address, `addr<T>` only); `ptr_size(T, p)` → `bits_of(T)`; managed references lose arithmetic/ordering and integer round-trips; `memcopy_unsafe`, address difference, address ordering and region copy are no longer provided (language specification §1.4); null-value error codes split by address space: `TC_RE_NULL_REFERENCE_DEREFERENCE` / `TC_RE_NULL_ADDRESS_DEREFERENCE` / `TC_RE_NULL_ADDRESS_ARITHMETIC` (no shared null-pointer code); new dangling-reference runtime code `TC_RE_DANGLING_REFERENCE` (reference lifetime semantics, §3.10.10); 87 diagnostic codes = 73 `TC_CE_*` + 14 `TC_RE_*`; the compiler standard gains the lowering contract (§1.4), local checkability (§1.5) and the call-depth bound (§8.10). The language specification carries present-tense clauses only; cross-version deltas live in this file.
- Removed historical process documents (0.0.44 conformance audit report and remediation ledger) and synced `docs/README`.

### Changed

- **0.0.45 gaps closed (2026-09-26)**: (1) managed references gain lifetime and dangling-reference semantics — a reference is identified by its **binding instance**, becomes invalid when that instance's lifetime ends, and `ref_load` / `ref_store` on an invalid reference report the new runtime code `TC_RE_DANGLING_REFERENCE` (language spec §3.10.10, §11.1, Appendix B.13; compiler standard §3.2, §6.7), raising the language code count from 86 to **87 = 73 `TC_CE_*` + 14 `TC_RE_*`** (implementation enum 88); (2) address arithmetic is defined to wrap modulo `2^n` in the target address width, the target word size (32/64) is added to the language spec's §1.3 implementation-defined list as item 4, and compiler standard §3.5 states that this edition fixes 64-bit targets; (3) diagnosis stages for grammar-restricted type positions are aligned across the board — `addr<ref<U>>` / `addr<struct>` / `addr<memblock>`, non-integer type parameters for bitwise/shift, comparison and logical type parameters, `void` memblock element types, non-integer `truncate` targets, and `void`/`addr<U>` type parameters of reference instructions are all **syntax rejections** (`TC_CE_SYNTAX`), matching the Appendix A productions; the same batch fixes the 0.0.44-era `let`-reference wording in language spec §3.10.3.
- **0.0.45 freeze review failed; back to freeze candidate (2026-09-26)**: `docs/TC语言标准设计说明书-0.0.45.md` and `docs/TC编译器标准设计说明书-0.0.45.md` were briefly treated as frozen text (status line, scope of effect, change policy, and the compiler standard's lockstep-versioning clause); review found three gaps that must be closed first, so the freeze was withdrawn: (1) a managed reference can outlive the binding it points to, leaving dangling references undefined; (2) the value semantics of address arithmetic and the target word size (32/64) are not pinned down; (3) grammar-restricted type positions disagree with the declared diagnosis stage and code. The `spec-0.0.45` tag has been withdrawn and both documents remain freeze candidates. Errata-only discipline still applies to any later freeze, and any semantic change requires a version bump; this file is the single errata channel and the separate errata checklist is no longer kept. Same batch: the language spec's §3.8.2 "table above" now cites the section explicitly, and the Appendix A comment uses the production names defined in Appendix A.
...
- **Documentation cleanup**: removed the closed-out historical process documents (0.0.41 analysis report / fix plan, 0.0.42 debt-cleanup plan) and the 0.0.42 specification text; renamed the errata checklist to the version-less `TC-语言标准一致性勘误清单.md` (it then covered both the 0.0.42 first round and the 0.0.44 second round; the checklist was later folded into the design documents and is no longer kept separately as of 0.0.45). Historical content can be recovered from release tags (e.g. `git show v0.0.42:docs/TC语言标准设计说明书-0.0.42.md`), as noted in `docs/README`; all 411 relative links across the repository were re-checked with zero dead links.
