...
- **语言规范升级至 0.0.44**（口径收敛版，不新增语言能力、不改变程序书写方式；诊断码仍 **86** 码 = 74 `TC_CE_*` + 12 `TC_RE_*`）。新增规范文本 `docs/TC语言标准设计说明书-0.0.44.md`、配套《TC-0.0.44-修订记录与兼容性说明》（变更记录 D1～D34）与《TC 语言标准一致性勘误清单》（首轮 63 项 ＋ 第二轮复审 27 项，全部处置）。IEEE 754-2019 保留为唯一外部规范性引用。
- **全部下游设计文档同步至 0.0.44**：编译器标准、VM 详设、VM 命令行参考、AOT 详设、Embed 详设、libtc 设计说明书统一改名为 `*-0.0.44.md`，更新规范基线行、术语（「语法阶段受限恢复」→「结构类语法阶段诊断」）与 D1～D34 变更点（接受集、诊断归属、语义澄清），并为每份文档新增「0.0.44 同步状态」标注（文档先行）。
...
- **文档面清理**：移除已收口的历史过程文档（0.0.41 分析报告／修复计划、0.0.42 遗留问题清零计划）与 0.0.42 规范文本；勘误清单改名为不带版本号的 `TC-语言标准一致性勘误清单.md`（现同时承载 0.0.42 首轮与 0.0.44 第二轮）。历史内容可用发布标签恢复（如 `git show v0.0.42:docs/TC语言标准设计说明书-0.0.42.md`），`docs/README` 已注明恢复方式；全仓 411 条相对链接复核无死链。
- 修正跨文档码数口径漂移：编译器标准「附录 B 85 → **86** 码」、libtc 4 处「85 语言码／合计 86」→「**86 语言码／实现枚举 87**」、`platform.md` 枚举规模 86 → **87**、`errors.md` 码数口径同步。

### Known issues

- **实现侧待同步**（规范与文档已先行，按《TC 语言标准一致性勘误清单》§9 跟进）：指针 `cast` 去掉等宽条件（W-6，当前实现仍拒绝非等宽所指类型）、`const_rhs` 嵌套调用改报 `TC_CE_SYNTAX`（W-26）、CT 类诊断须晚于全部 SEM 类诊断报告（W-27）。工具链行为版本仍为 **v0.0.43**，本次无实现行为变更。
