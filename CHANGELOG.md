...
- **语言规范升级至 0.0.44**（口径收敛版，不新增语言能力、不改变程序书写方式；诊断码仍 **86** 码 = 74 `TC_CE_*` + 12 `TC_RE_*`）。新增规范文本 `docs/TC语言标准设计说明书-0.0.44.md`、配套《TC-0.0.44-修订记录与兼容性说明》（变更记录 D1～D34）与《TC-0.0.42-语言标准一致性勘误清单》（首轮 63 项 ＋ 第二轮复审 27 项，全部处置）。IEEE 754-2019 保留为唯一外部规范性引用。
...
- **实现侧待同步**（规范与文档已先行，按《TC-0.0.42-语言标准一致性勘误清单》§9 跟进）：指针 `cast` 去掉等宽条件（W-6，当前实现仍拒绝非等宽所指类型）、`const_rhs` 嵌套调用改报 `TC_CE_SYNTAX`（W-26）、CT 类诊断须晚于全部 SEM 类诊断报告（W-27）。工具链行为版本仍为 **v0.0.43**，本次无实现行为变更。
