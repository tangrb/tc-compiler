...
- **0.0.45 设计草案（文档先行，尚未实现）**：新增 `docs/TC语言标准设计说明书-0.0.45.md` 与 `docs/TC编译器标准设计说明书-0.0.45.md`。相对 0.0.44 的差异：`ptr<T>` → `ref<T>`（托管引用）、新增 `addr<T>`（线性地址）与 `addr_*` 指令族、`nullptr` 拆为两个空值字面量：`none`（空引用，仅 `ref<T>`）与 `nil`（空地址，仅 `addr<T>`）；`ptr_size(T, p)` → `bits_of(T)`；托管引用去掉算术/序关系与整数互转；`memcopy_unsafe` 与地址差、地址序关系、区间拷贝均不再提供（语言标准 §1.4）；空值错误码按空间拆分：`TC_RE_NULL_REFERENCE_DEREFERENCE` / `TC_RE_NULL_ADDRESS_DEREFERENCE` / `TC_RE_NULL_ADDRESS_ARITHMETIC`（不再共用空指针码）；新增悬垂引用运行时码 `TC_RE_DANGLING_REFERENCE`（引用生命周期语义，§3.10.10）；诊断码 87 码 = 73 `TC_CE_*` + 14 `TC_RE_*`；编译器标准新增降级契约（§1.4）、局部可判定（§1.5）与调用深度上界（§8.10）。语言标准只承载现在时条款，跨版本差异记录于本文件。
- 移除历史过程性文档（0.0.44 符合性审计报告、0.0.44 符合性整改进度台账），`docs/README` 同步。

### Changed

- **0.0.45 三处缺口闭合（2026-09-26）**：① 托管引用新增生命周期与悬垂引用语义——引用以**绑定实例**为身份，实例生命周期结束时失效，`ref_load` / `ref_store` 对失效引用报新增运行时码 `TC_RE_DANGLING_REFERENCE`（语言标准 §3.10.10、§11.1、附录 B.13；编译器标准 §3.2、§6.7），语言码随之由 86 升至 **87 = 73 `TC_CE_*` + 14 `TC_RE_*`**（实现枚举 88）；② 地址算术结果定义为在目标地址宽度上按模 `2^n` 唯一确定，目标字长（32/64）补入语言标准 §1.3 实现定义清单第 4 项，编译器标准 §3.5 明确「本版固定 64 位」；③ 语法受限类型位置的诊断阶段全量对齐——`addr<ref<U>>` / `addr<struct>` / `addr<memblock>`、位运算/移位的非整数类型参数、比较与逻辑的类型参数、memblock 元素类型 `void`、`truncate` 非整数目标、引用指令的 `void`/`addr<U>` 类型参数一律按**语法拒绝** `TC_CE_SYNTAX`（与附录 A 产生式一致）；同批修正语言标准 §3.10.3 中源自 0.0.44 的 `let` 取引用措辞。
