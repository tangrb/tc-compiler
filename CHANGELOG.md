...
- **新增语言标准符合性审计**（[审计报告](docs/TC-0.0.44-语言标准符合性审计报告.md)）：以语言标准为唯一权威，核对 6 份设计文档与全部 `src/` 实现；确认 9 项既有未关闭项全部成立，并新增实现侧 70 项、文档侧 48+ 项、标准侧 4 项发现（含 2 项进程级故障：伪造指针越界读写、类型嵌套解析栈溢出）。审计未改动任何 `src/`、测试或既有文档。
