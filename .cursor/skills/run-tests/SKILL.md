...
description: >-
  ...
  ASan/UBSan/Valgrind, and CI sync scripts. Use when running tests, fixing
  failures, adding registrations, or debugging stdout/stderr mismatches.
  Counts: see test-map.md. Templates: tests-tc rule.
---

# Run TC-Compiler Tests

模板：Rule `tests-tc` · 账本：[test-map.md](../tc-architecture/test-map.md)（**规模数字唯一来源**）· 单元：Rule `unit-tests-c`

## 入口

```bash
make && bash scripts/run_tests.sh          # 全量 Gate
bash scripts/run_tests.sh --filter <名>    # 最小回归（仅 VM）
bash scripts/run_tests.sh --asan           # AddressSanitizer
make test                                  # 同 Gate；规模见 test-map.md
```
...
bash scripts/aot/run_tests.sh               # 注册/执行数见 test-map.md
make test-unit
...
python3 scripts/sync/check_doc_counts.py    # 校验 test-map 中 VM/AOT 数字
```
...
| AOT 不一致 | codegen vs `tc_aot_rt.c` shim；[gotchas.md](../tc-architecture/gotchas.md) |
...
4. 同步 [test-map.md](../tc-architecture/test-map.md)（跑 `check_doc_counts.py`）
