...
由 `@knowledge-graph` 索引指向；勿与其它 `kg-*.md` 同时整读。TcStmtKind 分发点表见索引 rule。

> **v0.0.39 多域 CFG（Phase 3+5）**：`TcCfgSet`（顶层 + 各函数独立域）、`MISSING_RETURN`、goto/label **仅函数内**；函数体内 goto **执行/AOT 已落地**。
...
警告：0.0.39 无语言警告种类，warnings 始终为空
```
