...
**规模**（`check_doc_counts.py` 校验）：**1125 VM** · **~496 AOT（注册）** / **496 AOT（执行）** · unit **~3205** `check()`。跑法：Skill `run-tests`。
...
| bitcast | fp_bitcast_roundtrip, bitcast_roundtrip32, bitcast_roundtrip64, let_bitcast_payload, **nan_canonical_bits**, **bitcast_ptr_nullptr** | bitcast_width_mismatch, bitcast_bool_type_mismatch, **bitcast_ptr_float** / **bitcast_float_ptr** / **bitcast_ptr_float32_category** / **bitcast_nullptr_float**（`ptr` ↔ 浮点静态拒绝：语言标准 §6.6.6、§3.10.9） |
...
AOT（`scripts/aot/run_tests.sh`）：**496** 注册项（`run_diff_test` + `run_check_ok/fail` + CLI golden）；**496** 执行通过项（另含 `run_runtime_fail`、embed codegen 等）。历史 Release Gate **272** 仅作基线参考。
