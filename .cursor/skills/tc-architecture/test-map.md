...
**何时读**：写/查 `.tc` 测试、确认某错误是否已有用例、查特性覆盖。**勿**为改 C 源码单独加载全文——用 `rg 测试名 scripts/` 更快。

> **现网规模（实现 v0.0.39 Phase 1–6 + Embed 0.0.39）**；目标规模见 `docs/TC-0.0.39-开发计划.md` §K.3。

规模：当前树约 **726 VM · ~318 AOT** · unit **约 3025** `check()`（含 embed ~943）；数字由 `scripts/sync/check_doc_counts.py` 与事实源校验。跑法见 Skill `run-tests`。
...
| STMT 24 / RHS 34 / 错误 90 | 同上 | inventory；`tc_error_kind_name` 白名单唯一性 |
...
| TC_ERR_FORMAT_* | format_string_error, format_type_mismatch*, format_operand_count* |
...
| format | format_output, format_hex_bin, format_bool, format_spec_all, io_extended | format_string_error, format_type_mismatch*, format_operand_count* |
...
| goto / label | （顶层 goto 已禁；函数内执行 Phase 5） | goto_outside_function, label_outside_function, goto_toplevel_*, goto_undefined, label_duplicate, goto_into_block, goto_sibling |
...
| test_types.c (276) | tc_types.c | check-types |
| test_symbol.c (67) | tc_symbol.c | check-symbol |
| test_io.c (127) | tc_io.c | check-io |
| test_bitwise.c (16) / test_shift.c (15) | tc_semantics.c | check-bitwise / check-shift |
| test_parser.c (139) | tc_parser.c | check-parser |
| test_analyzer.c (284) | tc_analyzer*.c + tc_cfg.c + tc_const_eval.c；静态 bool 三态、DFA、诊断优先级 | check-analyzer |
| test_diagnostic.c (33) / test_libtc.c (84) | diagnostic / libtc ownership | check-diagnostic / check-libtc |
| test_cfg.c (91) / test_executor.c (15) | CFG 静态条件边与逻辑读集 / executor | check-cfg / check-executor |
| test_stmt_index.c (18) | tc_stmt_index.h | check-stmt-index |
| test_warning.c (59) | tc_warning.c | check-warning |
| test_embed.c (584) / test_embed_aot.c (359) | tc_embed.c / tc_aot_codegen.c | check-embed / check-embed-aot |

AOT 差分（`scripts/aot/run_tests.sh`）：Release Gate **272**；当前树约 **327** = valid/stress/Phase5 stdout 差分 + static `--check` 双路径 + runtime stderr 对齐 + let 无槽代码生成断言 + CLI golden + embed codegen。

新用例注册 `scripts/vm/run_tests.sh`（+ AOT 如适用）；同步本文件 + `@knowledge-graph` 维护约定 + 对应 `kg-*.md` + `features.md`。
