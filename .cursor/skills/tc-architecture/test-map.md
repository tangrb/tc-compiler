...
| 覆盖 | 文件 / 入口 | 要点 |
|------|-------------|------|
| CLI `-I` / 无 REPL / 版本 0.0.44（黄金测试断言 `tc-vm 0.0.44`） | `scripts/vm/run_tests.sh` cli golden、`include_search_ok` | K1/K4 |
...
| 模块 4a～4d / Self / 签名 / 歧义导入 / 菱形拓扑 | `test_module.c` / check-module | 语言标准 §4.1～§4.5、§9.1；`diamond_import_*` |
| 模块错误 `.tc` | `tests/errors/module/`、`tests/modules/` | 无头、可见性、环、Self、成功 import（`--check`）；导入 struct 负例见 Phase 3 `imported_struct_*`；**`self_call_neg/`**（依赖模块内裸名 `funcall` → FUNCTION_SCOPE_ACCESS）、**`bare_scope_neg/`**（依赖模块内裸名成员读/赋值/输出/return/条件/`let` 六形态，错码须与作入口时一致） |
...
| bitcast | fp_bitcast_roundtrip, bitcast_roundtrip32, bitcast_roundtrip64, let_bitcast_payload, **nan_canonical_bits** | bitcast_width_mismatch, bitcast_bool_type_mismatch, **bitcast_ptr_float** / **bitcast_float_ptr** / **bitcast_ptr_float32_category** / **bitcast_nullptr_float**（`ptr` ↔ 浮点静态拒绝：语言标准 §6.6.6、§3.10.9）、**bitcast_nullptr_source** / **bitcast_nullptr_source_int**（A-4：`nullptr` 不参与 `bitcast` → `TC_CE_TYPE_MISMATCH`） |
| wrap | wrap_arithmetic_all, wrap_sub_mul, unary_wrap, unary_wrap_unsigned | wrap_mode_error*, abs_wrap_error |
| format | format_output, format_hex_bin, format_bool, format_spec_all, format_spec_flags, **format_spec_table**, **format_width_max**, **format_precision_max**, **format_precision_over_buffer**, io_extended | format_string_error, format_type_mismatch*, format_operand_count*, **format_specifier_***, **format_width_overflow** |
...
| Phase 6 CLI / API | cli version/help golden、`-I` include_search_ok；**cli -I path limit is not OutOfMemory**（规模上限用专用码，非 `OutOfMemory`） | include_search_ok（无 `-I` → import not found）；`import_ambiguous`（双 `-I`） |
...
| P6 测试补齐与门禁（2026-08-30） | **format_spec_table**（§10.4 负值位模式 / isize / `#` 备用 / `%-12f`）、**format_width_max**（65535 `--check`）、**format_precision_max**（`%.65535d` `--check`）、**format_precision_over_buffer**（`%.80d/%.200d/%.1000d` 运行期输出）、**fp_mod_edges**（有限 mod ±inf、-0.0） | **fp_mod_invalid_inf**、**fp_mod_invalid_before_divzero**、**memblock_type_count_zero**；unit：`test_types` 86 码唯一 + `%0008d`/`%65535d` + intern count=0 哨兵；`test_type_check` §10.2 顺序 + 命名 N 折叠；`check_doc_counts.py` 附录 B 85 = 实现 86−1 |
| **Self. 强制访问（2026-09-11）** | **self_access_ok**（+ `SelfAccessLib`：`Self.<名>` 的读/写/字段/`.count`/命名 N/`count:`/条件/return/输出）、**self_read_target**（+ `SelfReadLib`，`read(int32, Self.r)`） | **self_member_bare_name** 与 **self_bare_{io_operand,arith_operand,return,condition,assign_target,memblock_n,memblock_count}**（函数体内裸名访问模块 static → `FUNCTION_SCOPE_ACCESS: use Self.<名>`） |
| **static let 来源收紧（2026-09-11）** | **static_let_rule_ok**（+ `StaticLetRuleLib`：字面量 / 更早 `static let` 的裸名与 `Self.` 形式、字段读基址、`.count` 基址、命名 `N`（`Self.N` 与裸名）、`count:`） | **static_let_ref_var_rhs** / **\_operand** / **\_field** / **\_count** / **\_ctor** / **\_cast**（`static let` 引用 `static var` → `CONSTANT_EXPRESSION`，各位置）、**static_let_type_n_static_var** / **static_let_count_name_static_var**（命名 N / `count:` 来源为 `static var`） |
| **审计 §5 复核 + 观察项收口（本轮）** | 既有-2 `import_addr_{self,qual}`（+ AddrLib）· 既有-1 `import_member_{operand,struct}`（+ MemberLib/MemberUserLib）· 观察-④ `self_const_agg`（+ SelfConstAggLib）· `const_{struct,memblock}_copy` · 观察-⑤ `memblock_count_rhs`（+ MbCountLib）· 观察-⑥ `import_member_assign_{ok,dep}`（+ MemberAssignDepLib，含 MemberLib 的 MB） | `member_private_{read,field,addr,memblock,read_target}` · `SelfForeign{Priv,Pub,Field}Lib` · `import_member_{bad_qual,foreign_member}` · `member_assign_{readonly,private,type_mismatch,bad_qual}` · `import_addr_*` 无负例；unit：`test_type_check`（既有-4 断言改写） |
| **0.0.44 实现同步批（2026-09-10）** | **self_qual_operand**（+ SelfQualOperandLib，`Self.<名>` 限定标识符作运算/输出操作数）、**static_bool_cond**（+ StaticBoolCondLib，§5.2.2 原子集合三态判定）、**cond_ptr_compare** / **cond_readonly_field** / **cond_bool_rhs_forms**（条件接受指针比较、只读字段读取、`memblock_load` 等 bool RHS）、**ptr_cast_remark**（指针 `cast` 重标记：语言标准 §3.7、§3.10.5、§3.10.9）、**memblock_unsigned_suffix**（`N`/`count:` 的 `u`/`U` 后缀：语言标准 §3.8.1、§3.8.3）、**ptr_load_bool_normalize**（`ptr_load(bool)` 规范化：语言标准 §3.4、§6.8.2）、**static_var_chain**（+ StaticVarChainLib，`static var` 链式初始化：语言标准 §4.2） | **static_var_forward_ref** / **static_var_self_ref**（源序更晚/自身 → `UNDEFINED_VARIABLE`：语言标准 §4.2、§4.3）、**static_var_bare_member**（裸名非允许来源）、**static_var_init_div_zero**（准备阶段失败 → `TC_RE_*`）、**memblock_count_var_source**、**if_cond_funcall**（`funcall` 不可作条件 → 语法拒绝）、**ptr_cast_truncate** / **ptr_cast_non_ptr** / **ptr_cast_to_int**（余下指针 cast 拒绝）、**diag_priority_sem_before_ct** / **_static** / **_ct_over_derived** / **_ct_over_derived_program** / **_forward_over_ct**（语言标准 §11 四步规则阶段优先）；unit：`test_diagnostic` 挂起诊断槽、`test_analyzer` 优先级矩阵 |
