...
权威对照：语言标准附录 B（**86** 语言码，语言规范 0.0.44）· `docs/TC编译器标准设计说明书-0.0.42.md` §11.4（镜像）· 实现枚举：`tc_types.h`（**87** = 86 + `TC_ERR_OUT_OF_MEMORY`，`test_types.c` 断言）· 打印名见 `tc_error_kind_name()`
